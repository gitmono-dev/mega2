//! RM-ID: forward-only drop of `user_inbox_notifications`.
//!
//! In-app delivery already stopped writing this table (RM-IN). This
//! migration removes the leftover schema. Historical `m20260619*` CREATE
//! stays in the chain. `down` is a no-op: recovery is a new forward
//! migration, not a reconstructed inbox.
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(
                Table::drop()
                    .table(Alias::new("user_inbox_notifications"))
                    .if_exists()
                    .to_owned(),
            )
            .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn drop_user_inbox_notifications_after_apply_migrations() {
