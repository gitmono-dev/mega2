...
use crate::{
    ...
    jupiter::storage::webhook_storage::WebhookStorage,
};
...
impl WebhookService {
    pub fn new(storage: WebhookStorage) -> Result<Self, crate::common::errors::MegaError> {
    ...
    async fn deliver(
        ...
        event_type: &str,
        payload: &str,
    ) -> Result<(u16, String), crate::common::errors::MegaError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| crate::common::errors::MegaError::Other(e.to_string()))?;
...
pub fn validate_webhook_target_url(
    target_url: &str,
) -> Result<(), crate::common::errors::MegaError> {
...
pub fn encrypt_webhook_secret(secret: &str) -> Result<String, crate::common::errors::MegaError> {
    if secret.is_empty() {
...
pub fn decrypt_webhook_secret(secret: &str) -> Result<String, crate::common::errors::MegaError> {
...
fn parse_webhook_url(target_url: &str) -> Result<Url, crate::common::errors::MegaError> {
    Url::parse(target_url).map_err(|e| {
        crate::common::errors::MegaError::Other(format!("invalid webhook target_url: {e}"))
    })
}

fn load_webhook_secret_key_from_env()
-> Result<[u8; WEBHOOK_SECRET_KEY_LEN], crate::common::errors::MegaError> {
...
fn encrypt_webhook_secret_with_key(
...
) -> Result<String, crate::common::errors::MegaError> {
    let unbound_key = UnboundKey::new(&aead::AES_256_GCM, key_bytes).map_err(|_| {
        crate::common::errors::MegaError::Other("invalid webhook encryption key".to_string())
    })?;
    let key = LessSafeKey::new(unbound_key);

    let mut nonce_bytes = [0u8; WEBHOOK_SECRET_NONCE_LEN];
    SystemRandom::new().fill(&mut nonce_bytes).map_err(|_| {
        crate::common::errors::MegaError::Other(
            "failed to generate webhook secret nonce".to_string(),
        )
    })?;
    ...
    .map_err(|_| {
        crate::common::errors::MegaError::Other("failed to encrypt webhook secret".to_string())
    })?;
...
fn decrypt_webhook_secret_with_key(
    ...
    let encoded_payload = encrypted_secret
        ...
        .ok_or_else(|| {
            crate::common::errors::MegaError::Other(
                "invalid encrypted webhook secret prefix".to_string(),
            )
        })?;

    let payload = BASE64_STANDARD.decode(encoded_payload).map_err(|_| {
        crate::common::errors::MegaError::Other(
            "invalid encrypted webhook secret format".to_string(),
        )
    })?;
    ...
    let plaintext = key
        ...
        .map_err(|_| {
            crate::common::errors::MegaError::Other("failed to decrypt webhook secret".to_string())
        })?;

    String::from_utf8(plaintext.to_vec()).map_err(|_| {
        crate::common::errors::MegaError::Other(
            "decrypted webhook secret is not valid UTF-8".to_string(),
        )
    })
}

fn validate_webhook_url_static(url: &Url) -> Result<(), crate::common::errors::MegaError> {
    if url.scheme() != "https" {
        return Err(crate::common::errors::MegaError::Other(
    ...
    let host = url.host().ok_or_else(|| {
        crate::common::errors::MegaError::Other(
            "webhook target_url must include a host".to_string(),
        )
    })?;
...
async fn validate_resolved_ips(url: &Url) -> Result<(), crate::common::errors::MegaError> {
    ...
    let port = url.port_or_known_default().ok_or_else(|| {
        crate::common::errors::MegaError::Other(
            "webhook target_url must include a valid port".to_string(),
        )
    })?;

    let mut resolved_any = false;
    let addrs = lookup_host((host, port)).await.map_err(|e| {
        crate::common::errors::MegaError::Other(format!("failed to resolve webhook host: {e}"))
    })?;
...
mod tests {
    ...
    fn test_cl_payload_from_model_uses_contract_fields() {
        let model = crate::callisto::mega_cl::Model {
            id: 1,
