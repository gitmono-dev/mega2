...
| 配置 | 从示例生成本地 env（不提交）：`cp .env.test.example .env.test` |
...
cp -n .env.test.example .env.test   # 仅首次；已有则跳过
source .env.test

# 与 AGENTS.md / CI 对齐的本地测试门（不含 git-cli 时部分 IT 会失败，见下一节）
cargo test --all
```
...
```bash
docker compose -p monoengine-it -f docker-compose.test.yml exec postgres \
  pg_isready -U monoengine -d monoengine
...
```bash
# 1) 默认数据面（postgres / redis / rustfs；mailpit 仅供 website IT 捕获）
docker compose -p monoengine-it -f docker-compose.test.yml up -d --wait

# 2) 共享 git 工作根（必须先 mkdir，避免 Docker 以 root 建目录导致 EACCES）
...
# 3) 一次性建桶（profile init；不纳入默认 --wait）
docker compose -p monoengine-it -f docker-compose.test.yml \
  --profile init run --rm rustfs-init

# 4) Linux：拉起 git-cli（profile git）；否则 integration_git_cli 硬失败
docker compose -p monoengine-it -f docker-compose.test.yml \
  --profile git up -d --wait git-cli

# 5) 注入连接串并跑全量（含黑盒 + 模块集成）
...
```bash
docker compose -p monoengine-it -f docker-compose.test.yml \
  --profile git --profile app down -v
```
...
| Profile | 服务 | 何时启用 |
| --- | --- | --- |
| （默认） | `postgres`、`redis`、`rustfs` | monoengine 日常 IT 数据面 |
| （默认，可选消费） | `mailpit` | website 认证/产品邮件捕获；不是 monoengine 测试门 |
| `init` | `rustfs-init`（one-shot `mc`） | 建 `testbucket`；用 `run --rm`，不要指望它常驻 healthy |
...
# 黑盒：Vault / CLI / HTTP 等（monoengine bin crate）
cargo test -p monoengine --test integration_vault -- --nocapture --test-threads=1

# 黑盒：Git CLI 往返（需 --profile git 的 git-cli）
...
网络名固定为 `monoengine-test-net`：带 `-p monoengine-it` 与不带 `-p` 的两套栈会争用，启新栈前先 `down -v` 旧栈。
...
| `MONOENGINE_IT_GIT_UID` / `GID` | 容器内用户；本地 `id -u` ≠ 1000 时必设 |
...
- 是否在 Linux 上执行了 `--profile git up -d --wait git-cli`？
...
```bash
docker compose -p monoengine-it -f docker-compose.test.yml \
  --profile git --profile app --profile init down -v
