...
> **2026-09-10 更新（plan-20260906 SO-02 / SO-14 / SO-15 / SO-17）：** storage-only / trunk 形态的 compose 黑盒写门为 `scripts/git_protocol_smoke_storage_only.sh`：case **`HTTP trunk push`**（token tip 前进）、**`HTTP reject Git-client tag push`**（tag 拒且无残留）、**`HTTP reject unauthenticated push`**（默认 `push_auth=token` 下无凭据 push 必须失败）、**`HTTP trunk push (none)`**（`docker-compose-storage-only.auth-none.yml` + 无凭据 tip 前进）。对照 review smoke 的 CL 语义。
