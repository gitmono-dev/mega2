...
> **2026-09-10 更新（plan-20260906 SO-02）：** storage-only / trunk 形态的 compose 黑盒写门为 `scripts/git_protocol_smoke_storage_only.sh` 的 case **`HTTP trunk push`**：`MONOENGINE_GIT_SMOKE_PUSH=1` + Basic token URL（或 `MONOENGINE_IT_SEED_TOKEN` 注入）下断言远端 tip 相对 push 前前进；成功路径**不以**新建 `refs/cl/*` 为条件（对照 review smoke 的 CL 语义）。
