...
- 已补充 Noop mailer + test DB 风格的 dispatcher/storage 基线测试，并覆盖 dispatcher batch / max-in-flight、retry policy 配置生效、真实 SMTP/Mailpit 正路径和真实 SMTP 连接失败 retry；剩余是更完整 Mailpit/SMTP 故障矩阵与更长时间高水位矩阵。
...
| **可靠性与容错** | 基线已加固，仍需继续改进。字段支持 retry；dispatcher 已有可配置指数退避 retry/dead-letter、stale `sending` 恢复、可配置 bounded concurrency、真实 SMTP/Mailpit 正路径和真实 SMTP 连接失败 retry 覆盖，但仍缺告警、分布式锁/租约（多实例）和更完整 Mailpit/SMTP 故障矩阵。Dispatcher 失败不应导致通知永久丢失。 |
