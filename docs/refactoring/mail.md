...
**阶段 1（已接入，基线已加固）**：在 service 启动路径中 late-construct mailer 并 spawn dispatcher 已落地；构造失败静默忽略已改为可诊断错误；失败发送已具备可配置 retry + dead-letter disposition，并按 base/max 配置执行指数退避；dispatcher 已具备每 tick 可配置批次/并发限流、结构化汇总日志、stale `sending` 恢复、单 tick 批次背压测试、storage-level 并发 claim 竞争测试和真实 SMTP/Mailpit 正路径集成测试。剩余是完善 Mailpit 故障矩阵、长时间高水位背压验证和真实多进程 claim 竞争矩阵。
...
后续工作应严格按本计划 + config.md 的阶段划分进行：继续完善 Mailpit 故障矩阵、长时间高水位背压、真实多进程 claim 竞争、更高阶附件保留策略预设、模板版本审计/回滚和更完整运维面；如需支持运行期重新启用、SMTP/凭据重配或 `mail.template_*` 配置字段热替换，必须先设计动态重建与失败回滚语义；最后再持续扩展 provider、高级模板和运维能力。
