...
**阶段 5**：完整测试矩阵（坏 SMTP 已覆盖连接失败、dead-letter、协议拒绝、认证拒绝和缺失收件人 skip；仍需解析失败、权限失败、大量 pending job 背压、用户偏好全关场景）、在已有 Mailpit 正路径基线之上扩展 CI 中真实邮件发送干跑与故障矩阵、文档同步（README、部署指南）。
