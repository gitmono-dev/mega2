...
后续工作应严格按本计划 + config.md 的阶段划分进行：继续完善 Mailpit/SMTP 故障矩阵、长时间压力形态高水位背压、真实多进程/黑盒 claim 竞争、更高阶附件保留策略预设、模板版本审计/回滚和更完整运维面；如需支持运行期重新启用、SMTP/凭据重配或 `mail.template_*` 配置字段热替换，必须先设计动态重建与失败回滚语义；最后再持续扩展 provider、高级模板和运维能力。
