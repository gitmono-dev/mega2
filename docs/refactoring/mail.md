...
| 一级 `mail` 模块 (`src/mail/`) | **已激活** | `mod mail;` 在 `main.rs` 声明。`src/mail/mod.rs` 包含 `Mailer` trait、`NoopMailer`、`ConsoleMailer`、`SmtpMailer::new_with_password(...)`、`mailer_from_config(...)` + 构建消息 + 发送逻辑 + 单元测试。**2026-06-28 更新**：新增 `src/mail/testing.rs`，提供测试用 `MockMailer`（可捕获邮件并取出断言），补齐 mail.md 建议目录结构中的测试支持能力。旧 `src/email/` 降级为纯 re-export shim。 |
