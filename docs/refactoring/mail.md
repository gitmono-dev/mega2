...
| Profile / 热加载 / 集中校验对 mail 的支持 | **部分实现** | Profile、集中校验和 source warning 已接入 config 管线；热加载当前支持 `mail.enabled` true→false 关停 dispatcher，`mail.dispatcher_batch_size` / `mail.dispatcher_max_in_flight` 运行期调整 dispatcher 背压参数，`mail.retry_max_attempts` / `mail.retry_backoff_base_secs` / `mail.retry_backoff_max_secs` 运行期调整 retry/dead-letter 策略，`mail.attachment_prune_*` 运行期调整自动附件保留策略，`mail.template_*` 重建模板 registry，以及 `mail.provider`/SMTP 参数/凭据/from/starttls 经异步 mailer 重建热替换。重新启用 mail（`mail.enabled` false→true）仍要求重启。 |
...
- 运行期重新启用 mail（`mail.enabled` false→true）仍需重启；SMTP/凭据重配已通过动态 mailer 重建热生效，失败时保留旧 mailer。
...
后续工作应严格按本计划 + config.md 的阶段划分进行：继续完善 Mailpit/SMTP 故障矩阵、长时间压力形态高水位背压、真实多进程/黑盒 claim 竞争、更高阶附件保留策略预设和更完整运维面；运行期重新启用 mail（`mail.enabled` false→true）仍需单独设计 dispatcher 启动语义；最后再持续扩展必要的原生 provider、高级模板和运维能力。
