...
compose-backed integration targets, and the real website session check after
checking out the `orbit` and `website` siblings. Its comment on product email
is intentional: no local SMTP dependency is required for monoengine
notification paths.
