...
| `integration_api_write_trunk` | trunk 产品 API 写（plan-20260904 / AW-03；plan-20260917 LB-02..04）：token `create-entry` / `edit/save` 前进 tip、无 CL、无凭据 401；`delete_entry_*`（15 例）与 `move_entry_*`（13 例）覆盖目录删除 / 移动改名的双路径 401/403、traversal、ImportRepo 409、缺目录 404、`.gitkeep`、Review CL 复用与 tree 一致性；`tag_*`（7 例）覆盖 storage-only tag 路由挂载、trunk 写鉴权（401 / 403 / `push_auth=none` / Review 不加闸）、root 生命周期与运行时 OpenAPI。每个 case 各自 boot 一次真实 `service http`。Compose 黑盒对照：`scripts/api_write_smoke_storage_only.sh` 的 `delete-entry-git-visible` / `move-entry-git-visible` / `tags-list-create-delete` / `delete-entry-unauth-401`（LB-05） | PostgreSQL, Redis；raw HTTP（reqwest）；`push_policy=trunk` + `push_auth=token`；必须 `-- --test-threads=1` |
