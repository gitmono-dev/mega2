...
storage-only outbound events（plan-20260912）地址策略、HMAC 运输与有界 emitter 由 lib 测试（`jupiter::service::storage_event_transport` / `storage_event_emitter`）覆盖；WH-13 的 CLI/service 关停接线与 WH-11 的启动 secret 绑定由进程级 target `integration_storage_events_runtime` 覆盖（见上表）。生产运输没有 HTTP/私网逃逸开关。默认 disabled，不在业务提交路径挂钩（来源 hook 属 WH-03..08）。
