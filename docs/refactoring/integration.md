...
2. **Docker 测试栈已就位**：`docker-compose.test.yml`（固定项目名 `-p monoengine-it`）定义 PostgreSQL 15（host 15432）、Redis 7（16379）、Mailpit（11025/18025）、RustFS（19000/19001）、`profiles: ["git"]` 的 `git-cli`，以及 `profiles: ["app"]` 的常驻 **monoengine**（host `19180`，见 `docs/refactoring/test-infra.md`）；使用高位 host 端口避免冲突；无外部 Vault 容器（嵌入式 `VaultCore`）。cargo 黑盒仍用 `CARGO_BIN_EXE` 做 per-case 隔离。
...
| SecretRef | 已支持 `vault://secret/<name>#<field>`；`config secret` 可写入 `mail.password`、`redis.url`、`notification.slack.webhook_url`、`notification.webhook.token`、`object_storage.s3.access_key_id`、`object_storage.s3.secret_access_key`（见 `SUPPORTED_SECRET_FIELDS`）；对象存储 `vault://` 在启动路径、`config validate`、`config secret set/check`、`--resolve-secrets` 中均已对齐；**`redis.url` 的 `vault://` SecretRef 已在启动路径、`config validate`、`config secret check`、`--resolve-secrets` 中对齐（2026-06-28），解析后的值会被校验为合法 `redis://`/`rediss://` URL**；**（2026-06-30）新增黑盒用例 `config_secret_set_check_and_validate_resolve_redis_url_secret_ref`，覆盖 `config secret set/check redis.url` 与 `config validate --resolve-secrets` 的 Redis SecretRef 端到端链路。** | P0 覆盖 `ref/set/check/validate --resolve-secrets`、Redis URL SecretRef 与对象存储 S3 凭据 |
...
| 对象存储 SecretRef / 真实后端 | 启动路径解析有模块测试（透传/双 ref/缺失报错）；validate/CLI/`--resolve-secrets` 已与 S3 凭据对齐；P2 进程级黑盒 gate `config_secret_set_check_and_validate_resolve_object_storage_s3_secret_refs` 已落地，覆盖 `config secret set/check` 与 `config validate --resolve-secrets` 对 S3 凭据的端到端解析（不依赖真实 S3 服务端）。**2026-06-29 更新**：新增真实 S3-compatible 后端进程级 gate `integration_object_storage_s3_compatible_smoke`，通过 RustFS 容器执行 put/get/delete round-trip。 | GCS 凭据/真实后端 gate 仍为后续 | 复杂 |
...
- 错误链不输出 secret 明文。
- 当前支持 `mail.password`、`redis.url`、`object_storage.s3.access_key_id`、
  `object_storage.s3.secret_access_key` 等后置可迁移凭据；尝试 `database.db_url`
  必须被拒绝。
...
# 初始化 RustFS 测试 bucket（one-shot 服务，不纳入默认 --wait 集合）：
docker compose -p monoengine-it -f docker-compose.test.yml --profile init run --rm rustfs-init
