...
> **产品规则文档的反向声明——待本文档定稿后进行**：本文档在若干处标注了需要同步写入 `../monorepo.md` 的内容（按 N 分流的推送语义与 ADR-TP-18 的客户端对齐约定、不变式 I1–I6（含 I2a）、墓碑续接语义、文件路径索引的最终一致性）。按 `general.md` 的跨文档一致性要求，这些声明必须在两个文档中语义一致；在本文档定稿前 **不修改 `../monorepo.md`**，以免产品规则文档跟随草案反复变动。定稿后一次性同步，并在此处改为「已同步」。
