...
4. **mail provider 与 EmailDispatcher 已是 vault 之后的后置消费点，并已接入 SecretRef。** `src/context/mod.rs` 在 `VaultCore::new` 之后检查 `MailConfig` 的 `provider`、`password` / `password_ref` 互斥关系；若 SMTP provider 配置了 `password_ref`，通过 `VaultSecretResolver` 解析后再调用 `mail::mailer_from_config(...)`。SMTP 初始化失败会返回 `MegaError`，不再静默忽略；console provider 不读取 SMTP 凭据。**因此 `mail.password_ref` 是当前已落地的第一个配置侧 SecretRef 消费点**；后续不应重复做 resolver/mail 接入，只需治理明文兼容路径和真实 SMTP/Mailpit、多实例、metrics/lifecycle 等后续矩阵。
