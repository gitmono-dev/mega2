...
| 集中配置校验                   | **部分实现** | `src/config/validate.rs` 已提供 `Config::validate()` 首批入口，覆盖 `database.db_type`/`database.db_url`、`mail.password`/`mail.password_ref` 互斥、`mail.enabled` 必填项、Buck 限制，以及 object storage local/S3/S3-compatible/GCS 后端必填项；`Storage::new` 的 Buck 校验已改为返回 `MegaError`，不再 `panic!`；`config validate` 已对 `[oauth]`、`[mail].smtp_tls`/`[mail].tls` 和 raw TOML 任意未知字段输出 warning。端口范围、更多必填字符串、环境变量/profile 来源诊断和完整 source diagnostics 仍待补齐。 |
...
11. 已部分完成：新增 `src/config/validate.rs`，实现 `Config::validate()` 首批 hard error 校验，覆盖 `database.db_type`、`database.db_url` scheme、`mail.password` / `mail.password_ref` 互斥、`mail.enabled` 时 `smtp_host` / `from` 必填、Buck 限制，以及 object storage local/S3/S3-compatible/GCS 后端必填项；`config validate` 已复用该入口，`Storage::new` 的 Buck 非法配置已改为返回 `MegaError`。剩余端口范围、更多必填字符串等仍需继续补齐。
