...
> 6. **当前真正未落地的 config 主线工作**：`src/config/` 继续收敛剩余加载错误模型、扩展 redaction/SecretString 覆盖、profile 来源诊断、测试配置分层的全仓迁移、CI 配置校验矩阵、更多热加载真实消费端扩展。`config init`、Profile、基础样例凭据治理、`mail.password_ref` namespace hard error、raw TOML 未知字段 **hard error**（2026-06-23 已落地严格白名单检查）、`MEGA_*` 未消费/被忽略覆盖项 warning、source warning/source field/source override diagnostics（含首批 source-level 修复建议）、Vault ACL 级 config/generic SecretRef 权限矩阵、testing helper、reload 核心句柄、base/profile 文件 watcher、service watcher 生命周期、日志 reload subscriber、邮件 dispatcher 关停 subscriber、邮件 mailer/模板重配热加载 subscriber、Buck cleanup 调度 subscriber、artifact GC 调度 subscriber、订阅组件应用/回滚和 `Storage`/`AppContext` 访问路径已有首批可执行入口，mail 模板管理面已有审计/预览/持久化 upsert 与 registry 热替换基线；完整 profile/source diagnostics 矩阵已落地（2026-06-27）；后续只需围绕模板版本治理、样例校验矩阵、更多坏输入场景与热加载消费端继续收敛。
...
| 占位符 `${file:/path}` 文件挂载 secret | 已实现 | `src/config/expand.rs::expand_value` 识别 `${file:/path}`，读取文件内容作为字段值、去除单个末尾换行、按字面量使用不再二次展开（不进 `envsubst`）；与 `${var}` 混用或在路径内嵌套 `${...}` 会被拒绝（`FilePlaceholderMixed`），未闭合 `${file:` 被拒绝（`FilePlaceholderUnterminated`，路径脱敏）；读取失败返回 `FilePlaceholderRead` 脱敏诊断（含字段与文件路径，不含内容）；含单测覆盖正常展开、`$`/`${}` 内容不被再模板化、混用/路径嵌套/未闭合/缺失文件脱敏报错。 |
...
| `mail` / `MailConfig` / 邮件发送 | **已激活并接入 dispatcher 启动点** | 真实实现在一级模块 `src/mail/mod.rs`（`SmtpMailer`/`ConsoleMailer`/`Mailer`/`NoopMailer` + 单测，依赖 `lettre`），经 `src/main.rs:17` 的 `mod mail;` 编译；`MailConfig` 在 `src/config/model.rs`、`Config.mail` 字段已存在、`[mail]` 段已被消费，`mail.provider` 支持 `smtp`/`console`，dispatcher batch / max-in-flight 与 retry policy 支持运行期配置。`src/email/mod.rs` 为 re-export shim。`AppContext::new` 在 vault 之后解析 SMTP `mail.password_ref`（如存在）并通过 `mailer_from_config` 构造 provider mailer，构造失败现在返回可诊断错误。详见 `mail.md`。 |
| `[oauth]` 段                   | **已实现（2026-06-27）：强类型 `OAuthConfig` + 运行期消费者** | `Config.oauth: Option<OAuthConfig>`（`allowed_cors_origins: Vec<String>`）已落地，`oauth.allowed_cors_origins` 的 env list-parse 现映射到该字段；运行期消费者为 HTTP API 的 `CorsLayer`（`src/server/http_server.rs`：非空则用作 CORS allow-list，否则回退内置默认 origin）。`config validate` 校验每个 origin 非空且不含空白/控制字符；`oauth`/`oauth.allowed_cors_origins` 已进入 `known_fields` 白名单，故合法 `[oauth]` 不再被当作未知段告警（其它真正未知段/key 仍在加载期 hard error）。`config/config.toml` 附带 `[oauth]` 示例（`allowed_cors_origins = []`）。其它历史 `[oauth]` 子键（`campsite_api_domain`/`tinyship_api_domain`/`api_store_backend`、OAuth 回调地址等）若需要，待真实消费者出现时再逐项强类型化。 |
| `src/notification/`（邮件 outbox / dispatcher） | **已接入编译并在 mail 启用时启动 dispatcher，可靠性基线已加固** | `src/notification/{dispatcher,triggers,mod}.rs`（`EmailDispatcher`、触发器）+ `callisto::email_jobs` outbox 实体已从 mega 移植；`main.rs:18` 已声明 `mod notification;`。`AppContext::new` 在 vault 之后、`init_monorepo` 之前创建 `EmailDispatcher` 并 `tokio::spawn`；失败发送已有可配置 retry + dead-letter，dispatcher 已有可配置有界并发、stale `sending` 恢复、结构化 tick 汇总、批次背压测试和可配置自动附件保留清理；admin-only 作业管理 API 已支持 list/stats/failed retry/prune 和附件 metadata/下载/删除/按用户或事件类型收窄的保留期治理。剩余工作是真实 SMTP/Mailpit、多实例矩阵、更完整 metrics/lifecycle 和更多业务触发器接入。 |
...
| Profile / `config.<profile>.toml` | **已实现首批** | 全局 `--profile <name>` 优先于 `MEGA_PROFILE`；profile 文件固定为基础配置同目录、同 stem 的 `.<profile>.toml`，例如 `config.toml` → `config.prod.toml`；基础配置后叠加 profile，再叠加 `MEGA_*` 环境变量。profile 名限制为 ASCII 字母/数字/`-`/`_`，指定但文件不存在会报错；`config validate` 会对 base/profile 文件分别输出 raw TOML warning，profile 类型冲突会报 profile 路径、字段路径、期望类型和脱敏建议；`config secret set/check` 仍走最小 DB/Vault bootstrap，但读取 profile 合并后的 DB 配置。**（2026-06-27）跨 base/profile/env 的完整 source diagnostics 矩阵已落地并由 `source_diagnostics_*` 单测锁定**（见「`monoengine config` 命令族」行）。 |
| `monoengine config` 命令族      | **部分实现** | CLI 已支持按命令 `LoadMode` 加载；`config init`、`config secret ref/set/check/rotate`、`config vault reset/rekey/backup/restore` 与 `config validate --resolve-secrets` 已实现；`config validate` 已切到 `LoadMode::RawSources` 并在命令内解析配置，已输出 raw TOML 和 `MEGA_*` 环境变量未消费/未知字段 warning；文件级 warning 已保留可测试的 source path，base/profile/env warning 已能汇总，并可通过 `config validate --deny-warnings` 升级为失败；`config validate --show-sources` 已能展示 base/profile/env warning、字段来源图和覆盖关系，不输出原始值；坏 env/profile 类型会报出来源、字段路径和期望类型，并脱敏原始值；首批诊断已包含移除/替代字段、覆盖来源清理、字段来源编辑建议、敏感字段 source 提示和数组字段 override 的 replace-not-append 提示。**（2026-06-27）跨 source / profile 诊断矩阵已完整覆盖**：base→profile→env 的覆盖链（含同一字段三源链）、每字段的来源图（base/profile/env 全部归因）、数组 inline-table 元素级路径与各源 warning 均已实现并由 `source_diagnostics_*` 单测（含 canonical `source_diagnostics_full_cross_source_matrix_for_single_field`）锁定，输出不泄露原始值。**（2026-06-28）source diagnostics 敏感字段提示已扩展至 `notification.slack.webhook_url_ref` 与 `notification.webhook.token_ref`**，与 `mail.password_ref` 一致，避免诊断输出中泄露这些 Vault SecretRef 路径；剩余只是更多坏输入场景的增量补充。 |
| 集中配置校验                   | **部分实现；未知字段 hard error 与数组元素级 source diagnostics 已落地** | `src/config/validate.rs` 已提供 `Config::validate()` 首批入口，覆盖 `database.db_type`/`database.db_url`、数据库连接池参数（`max_connection`、`min_connection`、`acquire_timeout`、`connect_timeout`）、`monorepo.import_dir`/`root_dirs`/`admin` 空值、`monorepo.rename.similarity_threshold` 范围、`pack` 大小/缓存路径/通道容量、`blame` 阈值、`log.level`、`lfs` 路径/URL、`build.orion_server`、`redis.url`、`mail.password`/`mail.password_ref` 互斥、`mail.enabled` 必填项、`mail.dispatcher_batch_size`/`mail.dispatcher_max_in_flight` 非零、`mail.retry_max_attempts`/`mail.retry_backoff_base_secs`/`mail.retry_backoff_max_secs` 非零且 retry backoff 上限不小于基础值、`mail.attachment_prune_*` 非零/状态安全、Buck 限制、artifact GC 调度参数、object storage local/S3/S3-compatible/GCS 后端必填项、S3 access key/secret key 的 `vault://` SecretRef namespace 校验（合法 `vault://secret/config/<profile>/object_storage/access_key_id#<field>` 与 `.../secret_access_key#<field>` 通过，错误 namespace 不回显 vault path）、`sidebar.default_items` 关键字段/重复 public_id，以及可选 `orion_server` 的端口/URL/DB URL；`Storage::new` 的 Buck 校验和 BuckService 构造失败已改为返回 `MegaError`，不再 `panic!`；`config validate` 已对 `[oauth]`、`[mail].smtp_tls`/`[mail].tls`、兼容期 `[mail].password`、raw TOML 任意未知字段和 `MEGA_*` 未消费/被忽略覆盖项输出带修复建议的 warning，且 env/profile 类型错误已脱敏并带修复建议；source warning 已可作为错误门禁，source field/source override 已可显式展示并带首批 source-level 修复建议和敏感字段 value-omission 提示。**2026-06-23**：新增 `validate::reject_unknown_fields`，在 `Config::new`/`load_str`/profile 文件加载路径上对 raw TOML 执行严格白名单检查，真正未知的段/key 在加载期直接返回 hard error（同时保留 `config validate` 的 warning 诊断）；`[oauth]` 作为遗留段被跳过，继续由 source diagnostics 输出 warning。**2026-06-23**：source field/source override 诊断已扩展到数组 inline table 的每个元素字段（如 `sidebar.default_items[0].public_id`），覆盖关系与数组替换提示同样按元素粒度输出。**（2026-06-27）跨 source / profile 诊断矩阵已完整（base/profile/env 覆盖链 + 每字段来源图 + 数组元素 + 各源 warning），由 `source_diagnostics_*` 单测锁定；**（2026-06-28）敏感字段 value-omission 提示已覆盖 `notification.slack.webhook_url_ref` 与 `notification.webhook.token_ref`**；后续仅为更多坏输入场景的增量补充。**
| SecretRef + 运行期 resolver    | **已实现首批；对象存储 S3 SecretRef 已扩展至 validate/CLI**  | `SecretRef`、`SecretResolver`、`VaultSecretResolver` 已编码；支持 `vault://secret/...#field`、缓存 TTL、`evict`/`evict_all`，并实现 `mail.password` / `mail.password_ref` 互斥；`config validate` 与 `config secret ref/set/check` 已对 `mail.password_ref` 和 `object_storage.s3.access_key_id` / `secret_access_key` 的 namespace 做 hard error 校验，只允许 `vault://secret/config/<profile>/mail/password#<field>`、`.../object_storage/access_key_id#<field>` 与 `.../object_storage/secret_access_key#<field>`；`config validate --resolve-secrets` 现已解析 object_storage S3 SecretRef；真实 Vault resolver 已覆盖 secret 缺失、字段缺失和字段非字符串的脱敏错误；Vault ACL 已覆盖 config/generic token 互斥矩阵，generic token 显式拒绝 `secret/config/*`。 |
| 测试配置辅助                   | **已实现首批并继续迁移现有测试** | `src/config/testing.rs` 已提供 `TestConfigBuilder`、`isolated_config()` 与 `TestSecretResolver`，可派生临时 base/cache/LFS/object storage 路径、接收 `.env.test` 风格的 DB/Redis/mail SecretRef 覆盖，并用内存 resolver 覆盖 secret 读取/缺失/evict 场景；CLI `parse` 的无子命令加载单测已改用临时配置文件，`commands::config` 中涉及 validate/SecretRef 的命令层测试、`config::validate` 校验矩阵、`Storage::mock()` 和 `jupiter::tests::test_storage()` 已迁到 `isolated_config()`；`Config::mock()` 已收窄为 test-only helper，避免作为运行期构造入口继续扩散。`testing::EnvVarGuard` 现支持 `remove()`（在持锁期间临时移除某个 `MEGA_*` 环境变量并在 drop 时还原），供文件/Profile 优先级单测在不受 `.env.test` 注入的 `MEGA_DATABASE__DB_URL` 等环境覆盖干扰的前提下断言；`env_lock()` 也改为从中毒锁恢复（`unwrap_or_else(into_inner)`），避免单个测试 panic 把 `PoisonError` 级联到其它 env 串行化测试。尚未把全仓所有业务测试都迁移到该 helper。 |
| CI 配置样例校验                | **已实现首批** | `.github/workflows/config-validation.yml` 已新增 sibling-aware 配置验证入口，会 checkout `monoengine` 与 `orbit`，运行格式检查、Clippy lint（`cargo clippy --all-targets --all-features -- -D warnings`，对齐 general.md 第 297/310 行强制的 clippy 验收门禁）、配置模板/loader/profile/env diagnostics 单测、warning-as-error 单测、source warning/field/override 脱敏和修复建议单测、坏 TOML 源行脱敏 smoke、坏 env/profile 类型脱敏单测、monorepo/pack/blame/sidebar/artifact GC 坏参数单测、缺失 `mail.password_ref` 与测试 resolver 权限拒绝的命令层脱敏单测、`mail.password_ref` 错误 namespace 脱敏单测与 CLI smoke、兼容期 `mail.password` source warning 单测与 CLI smoke、profile 引入废弃字段的 `--show-sources`/`--deny-warnings` smoke、profile merge CLI 普通/`--show-sources` 校验（含敏感 `database.db_url` 覆盖脱敏和数组替换提示）、仓库基础样例 `config validate`、`config init` 生成结果普通/`--deny-warnings` 校验，以及坏占位符、坏 SecretRef URI、坏 Redis URL scheme 的 CLI 失败 smoke；真实 Vault resolver 也已覆盖 secret 缺失、字段缺失和字段非字符串脱敏单测，VaultCore ACL 单测已覆盖 config/generic runtime token 隔离。后续继续补更多坏输入和完整 source diagnostics 场景。 |
| 受控热加载                     | **已实现核心基础 + 文件 watcher + service 生命周期 + 日志/邮件/Buck cleanup/artifact GC 订阅首批接入**  | `src/config/reload.rs` 已提供 `ConfigHandle`、`reload_from_path` 复用 base/profile 加载流水线构建候选配置、候选 `Config::validate()`、日志字段白名单热更新、`mail.enabled` 从 true 到 false 的运行期关停、`mail.dispatcher_batch_size`/`mail.dispatcher_max_in_flight` 运行期热更新、`mail.retry_max_attempts`/`mail.retry_backoff_base_secs`/`mail.retry_backoff_max_secs` 运行期热更新、`mail.attachment_prune_*` 运行期热更新、已运行 Buck cleanup 任务的 enable/interval/retention 热更新、已运行 artifact GC 任务的 interval/grace/batch 热更新和 `enable=true` 到 `false` 关停、数据库/Redis/Buck 上传限制/邮件重启类字段需重启报告；base_dir、monorepo、pack、lfs、blame、build、object_storage、orion_server、sidebar 等静态消费端字段也已保守报告需重启且不发布候选快照、不泄露对象存储密钥；`mail.password_ref` 变更经 mailer 异步重建热加载（vault 后重新解析）、失败保留旧 mailer 且不泄露引用，对象存储等 bootstrap 类 SecretRef 不可配置；以及 `ConfigReloadSubscriber` 订阅应用/逆序回滚语义；`ConfigReloadWatcher` 已可轮询 base/profile 文件签名变化、触发 reload、拒绝坏配置并继续运行；`service` 命令已在服务生命周期内启动 watcher 并在子命令退出后停止，且已注册日志 reload subscriber 使 `log.level` / `log.print_std` / `log.with_ansi` 变更先应用到 tracing layer 再发布配置快照；`AppContext` 在邮件 dispatcher 已启动时会注册 dispatcher 订阅，使 `mail.enabled = false` 停止处理待发送队列，并使 dispatcher 限流、retry policy 和附件保留策略字段热更新；邮件 provider/SMTP 参数/`username`/`password`/`password_ref`/`from`/`starttls`/模板字段重配已**运行期热加载**：模板 registry 由 `config_reload_mail_template_subscriber` 同步重建（失败则整次 reload 回滚、保留旧快照与旧 registry、不发布），SMTP mailer 由 `config_reload_mailer_subscriber` 异步重建（vault 后重新解析 `password_ref`，快照发布后换入，失败保留旧 mailer），全程不泄露明文或 SecretRef 路径；仅整体新增/移除 `[mail]` 段仍要求重启（`mail.enabled` true↔false 已支持运行期热加载），均有矩阵测试；HTTP service 在 Buck cleanup 与 artifact GC 已启动时会注册调度 subscriber，使各自调度字段和 `enable=false` 生效。`Storage`/`AppContext` 已持有共享 `ConfigHandle`，`storage.config()` 会读取 handle 当前快照。邮件重配的动态 mailer 重建与失败保留旧 mailer 语义已落地（mail.md 阶段 4）。 |
...
- `Storage::config()` 已改为从 `Storage` 持有的共享 `ConfigHandle` 读取当前快照，锁异常时回退初始 `Arc<Config>`，不再存在 `Weak::upgrade().expect("Config has been dropped")` 的 panic 点；`AppContext` 也持有同一个 `ConfigHandle` 并提供 `config()` 快照方法。`ConfigReloadSubscriber` 已补首批订阅应用/失败回滚语义，`ConfigReloadWatcher` 已补 base/profile 文件轮询触发 reload 语义，`service` 命令已补 watcher 生命周期接线并注册日志 reload subscriber；邮件 dispatcher 已补 `mail.enabled = false` 的运行期关停和 dispatcher 限流更新 subscriber；HTTP Buck cleanup 和 artifact GC 任务已补调度参数热更新和运行期关停 subscriber；邮件 mailer 重建 subscriber（provider/SMTP/凭据变更后异步重建并重新解析 `password_ref`）与模板 registry 重建 subscriber 也已接入。后续热加载仍需补更多真实消费端订阅接入；`[mail]` 段整体增删继续明确为重启（`mail.enabled` true↔false 已运行期热加载）。
- `AppContext::new` 当前已返回 `Result` 并传播 `Storage::new`（含数据库连接和 migration）、Redis 初始化、`VaultCore::new`、`init_monorepo` 与 mailer 初始化错误；HTTP 服务监听地址解析/绑定失败也已从 `unwrap` 改为经 `service http` / `service multi` 返回错误，静态 CORS header 也不再通过运行期 `unwrap()` 构造。邮件发送失败已具备有界 retry + dead-letter 基线；剩余风险主要在 dispatcher 生命周期治理、并发和后台任务可观测性。
- `mail.password` 明文字段仍为兼容期入口；`SecretString` 防误打印、运行期 deprecation warning、source diagnostics warning、`--deny-warnings` 门禁和 CI smoke 已落地，后续继续按兼容期治理推动生产配置使用 `mail.password_ref`。
...
1. **引导循环决定数据库凭据无法使用本项目 Vault SecretRef。** 数据库凭据用于 DB-only `VaultCore` bootstrap，在 vault 就绪前已被消费，无法反过来从 vault 解析——这是不可破的引导循环，必须通过环境变量、文件挂载 secret 或部署平台 secret 机制注入。**（2026-06-28 更新）Redis URL 已在 vault 就绪后接入 SecretRef**：`AppContext::new` 的 `resolve_redis_url_secret` 解析 `redis.url` 中的 `vault://` SecretRef（合法 namespace 为 `vault://secret/config/<profile>/redis/url#<field>`），字面量 URL 原样透传；`config validate` 与 `config secret set/check` 现已接受并校验合法 namespace 下的 `redis.url` SecretRef。对象存储 S3 凭据在 DB-only vault bootstrap 后已可走 `vault://` SecretRef：`AppContext::new` 的 `resolve_object_storage_secrets` 在 vault 就绪后解析 `object_storage.s3.access_key_id` / `secret_access_key` 中的 `vault://` SecretRef（vault.md/orbit.md 阶段 G），字面量凭据原样透传；`config validate` 与 `config secret set/check` 现已接受合法 namespace 下的 object_storage SecretRef，`config validate --resolve-secrets` 也会解析它们。
...
4. **新增 SecretRef 消费端前必须先证明其消费点晚于 vault。** 已落地并经 `config validate`/`--resolve-secrets` 支持的配置侧 SecretRef 消费端包括 `mail.password_ref`、`redis.url`、`notification.slack.webhook_url_ref`/`notification.webhook.token_ref`，以及对象存储 `object_storage.s3.access_key_id`/`secret_access_key`（2026-06-28 已对齐 validate/CLI/`--resolve-secrets`）；这些 SecretRef 字段在 source diagnostics 输出中均已作为敏感字段回退处理（含 `notification.slack.webhook_url_ref` 与 `notification.webhook.token_ref`，2026-06-28）。任何新增消费端都必须通过依赖表确认其初始化晚于 vault，不能违反启动顺序。
...
| CLI 命令 | `config init`、`config validate`（`--resolve-secrets`/`--deny-warnings`/`--show-sources`）、`config secret ref/set/check/rotate`、`config vault reset/rekey/backup/restore` 已落地 | 更丰富的错误场景覆盖与生成结果校验 | 中等 |
...
**已编译且活跃：**
- 一级模块 `src/mail/mod.rs`：`Mailer` trait、`NoopMailer`、`ConsoleMailer`、`SmtpMailer` 与 `mailer_from_config` 工厂，由 `src/main.rs:17` 的 `mod mail;` 接入。
- `MailConfig`（`src/config/model.rs`，`#[serde(default)]` 经 `Config.mail` 字段进入统一加载管道）、`config/config.toml` 的 `[mail]` 段已被消费；`provider` 支持 `smtp`/`console`，`password_ref` 已作为 `Option<SecretRef>` 接入，并与 `password` 互斥且只适用于 SMTP。
- `src/context/mod.rs`：在 `VaultCore::new` 之后从 `config.mail` 构造 provider mailer；如 SMTP 配置存在 `mail.password_ref` 则先通过 `VaultSecretResolver` 解析；构造失败会返回 `MegaError`，不再静默降级。
...
config secret set/check/rotate    # LoadMode::VaultBootstrap，只解析配置路径并走最小 DB/Vault bootstrap
                                  #   rotate 把 mail.password 写入 vault；提示运行中消费方重启，新解析与
                                  #   config validate --resolve-secrets 立即使用轮换后的值（不自行触发 mailer 重建）
config vault backup <DEST>        # LoadMode::VaultBootstrap，备份 core_key.json 到目标位置并写入 .meta.json
config vault restore <SRC> --force# LoadMode::VaultBootstrap，验证备份 key 能解封当前数据库后原子替换 core_key.json
...
- `src/config/expand.rs`：实现 `${base_dir}` 等字符串占位符展开，以及 `${file:/path}` 文件挂载 secret 占位符（读取文件内容、去末尾换行、内容按字面量不再二次展开）；首批失败路径已返回 `ConfigError`，未解析/非法占位符值与文件读取失败已输出来源、字段路径和修复建议（不含值/文件内容），后续 source diagnostics 仍需补跨 source 覆盖关系。
...
4. 对部分列表类字段进行逗号分隔解析（如 `oauth.allowed_cors_origins`、`monorepo.admin`、`monorepo.root_dirs`）。`oauth.allowed_cors_origins` **已（2026-06-27）映射到强类型 `OAuthConfig.allowed_cors_origins`** 并由 HTTP API 的 `CorsLayer` 消费（见 `[oauth]` 段说明）。
5. 在反序列化前执行字符串占位符替换。
6. 将最终配置反序列化为强类型 `Config`。

占位符替换主要用于让配置值引用基础目录等公共路径，例如 `${base_dir}`。该机制工作在字符串值上，适合路径类配置复用，但不适用于非字符串字段。此外还支持 `${file:/path/to/secret}` 占位符（`expand.rs`），把文件挂载 secret 的内容读入字符串字段（去除单个末尾换行，内容按字面量使用、不再二次展开），用于 K8s Secret volume、systemd `LoadCredential=` 等场景。
...
- `mail`：邮件通知配置，`Option<MailConfig>`。当前 `mail.provider` 支持 `smtp` / `console`，同时支持兼容期明文字段 `mail.password: Option<SecretString>` 与推荐字段 `mail.password_ref: Option<SecretRef>`，两者互斥且只适用于 SMTP；`mail.dispatcher_batch_size` / `mail.dispatcher_max_in_flight` 控制 outbox dispatcher 背压，`mail.retry_*` 控制 retry/dead-letter 策略，`mail.template_default_locale` / `mail.template_dir` 控制启动期外部 TOML 邮件模板覆盖，`mail.attachment_prune_*` 控制 outbox 附件自动保留清理；`password_ref` 是首个已落地的配置侧 SecretRef 消费点。

> **字段现状（mail 已存在；oauth 已新增 `allowed_cors_origins`，2026-06-27）：**
> - **`mail` / `MailConfig`：已是 `Config` 字段且已定义。** `MailConfig` 在 `src/config/model.rs`（扁平结构 `enabled`/`provider`/`smtp_host`/`smtp_port`/`username`/`password`/`password_ref`/`from`/`starttls`/`dispatcher_batch_size`/`dispatcher_max_in_flight`/`retry_max_attempts`/`retry_backoff_base_secs`/`retry_backoff_max_secs`/`template_default_locale`/`template_dir`/`attachment_prune_enabled`/`attachment_prune_interval_secs`/`attachment_retention_days`/`attachment_prune_statuses`，provider 默认 `smtp`，端口/STARTTLS 默认值经 `default_smtp_port`/`default_starttls` 提供，dispatcher 默认值为 50 / 8，retry 默认值为 5 次 / 30s / 300s，模板默认 locale 为 `en-US` 且外部模板目录默认不启用，附件自动清理默认关闭）。`config/config.toml` 的 `[mail]` 段**已被消费**；段内 `smtp_tls`/`tls` 属未知 key，`config validate` 已对 TOML 和 `MEGA_MAIL__TLS`/`MEGA_MAIL__SMTP_TLS` 输出首批 warning。真实 `SmtpMailer::new_with_password(...)` / `ConsoleMailer` 经 `AppContext::new` 在 Vault 就绪后通过 `mailer_from_config` 调用，模块经 `main.rs:17` 的 `mod mail;` 编译（**不是** `mod email;`；`src/email/mod.rs` 仅为 re-export shim）。`mail.password_ref` 已落地并通过 `VaultSecretResolver` 解析；明文 `mail.password` 已完成首批兼容期治理，并已纳入 source diagnostics warning。
> - **`oauth` / `OAuthConfig`：已是 `Config` 字段（2026-06-27）。** `Config.oauth: Option<OAuthConfig>`，`OAuthConfig.allowed_cors_origins: Vec<String>`；env list-parse `oauth.allowed_cors_origins`（`src/config/source.rs`）现映射到该字段。运行期消费者为 HTTP API 的 `CorsLayer`（`src/server/http_server.rs`）：列表非空时作为 CORS allow-list，否则回退内置默认 origin。`config validate` 校验各 origin 非空且不含空白/控制字符，`oauth`/`oauth.allowed_cors_origins` 已进入 `known_fields` 白名单。其它历史 `[oauth]` 子键（`campsite_api_domain`、`tinyship_api_domain`、`api_store_backend`、OAuth 回调地址校验、`[oauth.github]` 示例等）尚未强类型化，待出现真实消费者时再逐项落地。

这种拆分方式让上层调用方可以只依赖自己需要的配置域。但需要注意：**`config/config.toml` 中仍存在未被强类型字段消费的内容：`[oauth]` 段的 `allowed_cors_origins` 现已被 `OAuthConfig` 消费，但其历史子键（`campsite_api_domain`/`tinyship_api_domain`/`api_store_backend`）仍是白名单容忍但不消费的遗留 key；以及已消费段内的未知 key（如 `[mail]` 中的 `smtp_tls`/`tls`，`config.toml:275-276`）；用户自行增加的未知段或未知 `MEGA_*` 覆盖项同样会被 serde/config crate 忽略，配置来源与 Rust 结构之间并非严格一一对应。** 这既是当前的技术债务，也是安全/运维隐患（用户以为写了某段就生效了）。`config validate` 已基于 raw TOML 对 `[oauth]`、`[mail].smtp_tls`/`[mail].tls`、兼容期 `[mail].password` 和任意未知字段输出 warning，并对 `MEGA_OAUTH__...`、`MEGA_MAIL__TLS`/`MEGA_MAIL__SMTP_TLS`、`MEGA_MAIL__PASSWORD` 与未知 `MEGA_*` 覆盖项输出不含值的 warning；后续仍需扩展 profile 合并来源、脱敏原始值和修复建议，并考虑在模型上使用 `#[serde(deny_unknown_fields)]` 的白名单模式或后置 key 检查——注意 `MailConfig` 当前**未**使用 `deny_unknown_fields`，并保留“丢弃未知字段”的兼容注释，与该建议存在张力，需在落地时统一策略。见「推荐加载流水线」。

`Config` 已提供若干加载/测试入口：`Config::load_str()`、`Config::load_sources()`（`src/config/mod.rs`）以及 `src/config/testing.rs` 中的 `TestConfigBuilder`/`isolated_config()`。`Config::mock()` 已收窄为 test-only helper，后续业务测试优先使用隔离配置 helper，而不是把裸默认配置当作运行期构造入口。
...
- 邮件通知（已编译，dispatcher 已在 mail 启用时启动）：一级模块 `src/mail/mod.rs` 提供 `SmtpMailer`/`ConsoleMailer`/`NoopMailer`，`MailConfig` 已定义并接入 `Config`；`src/context/mod.rs` 已在 vault 之后解析 SMTP `mail.password_ref`，再构造 provider mailer 并 spawn `EmailDispatcher`。作为首个配置侧 SecretRef 消费点，基础迁移已完成；明文 `mail.password` 的 deprecation warning、`SecretString` 包装和 source diagnostics 门禁已落地，发送失败的可配置 retry + dead-letter、有界并发、dispatcher 限流配置、stale `sending` 恢复和作业管理 API 基线已落地，剩余工作是兼容期收尾治理、真实 SMTP/Mailpit、多实例矩阵和更完整 metrics/lifecycle（见「已落地的 mail/notification 子系统现状」）。
...
1. **`AppContext::new`（`src/context/mod.rs:51`）** 的启动顺序已重构为：
   - 第 55–56 行 `database_connection(&config.database)` 建立数据库连接并执行 migration；连接或 migration 失败会返回 `MegaError`，不再 panic。
   - 第 60–71 行 `VaultCore::from_database_connection(db_connection, key_path)` 做 **DB-only vault bootstrap**，vault 在此就绪。
   - 第 75–79 行 `resolve_object_storage_secrets(...)` 解析 `object_storage.s3.access_key_id` / `secret_access_key` 中的 `vault://` SecretRef，随后 `build_object_storage(...)` 构造对象存储；因此对象存储凭据在 vault 就绪后解析，不再属于早期运行时依赖。
   - 第 81–86 行 `Storage::new_with_connection(config, db_connection, object_store)` 复用同一 DB 连接和已解析对象存储构造完整 Storage。
   - 第 88 行 `init_connection(&config.redis)` 在 vault/Storage 之后执行；Redis URL 格式和连接失败会返回脱敏错误，不再 panic。
...
2. **`VaultCore::from_database_connection` 就绪** 之后消费的配置字段才可纳入可迁移凭据。
3. **mail provider 与 EmailDispatcher 已是 vault 之后的后置消费点，并已接入 SecretRef。** `src/context/mod.rs:95-231` 在 vault 就绪后检查 `MailConfig` 的 `provider`、`password` / `password_ref` 互斥关系；若 SMTP provider 配置了 `password_ref`，通过 `VaultSecretResolver` 解析后再调用 `mail::mailer_from_config(...)`。SMTP 初始化失败会返回 `MegaError`，不再静默忽略；console provider 不读取 SMTP 凭据。**`mail.password_ref` 与 notification slack/webhook token 是当前已落地的配置侧 SecretRef 消费点**；对象存储 S3 凭据也已落地（2026-06-28）。后续不应重复做 resolver/mail 接入，只需治理明文兼容路径和真实 SMTP/Mailpit、多实例、metrics/lifecycle 等后续矩阵。
4. **`ssh_server_key`、PGP、Nostr** 已由 vault 管理，但属于 vault 内部 secret，不在 `Config` 结构体中。SSH server key 读取/生成/写入失败、PGP key 读取/解析/保存/删除、Nostr key 读取/生成/解析已改为返回可诊断错误；它们不改变配置侧 `SecretRef` 的边界。

> 结论：数据库凭据、vault 自身 key material 属于不可破的引导配置；Redis URL 当前仍在 vault 就绪后连接，但尚未接入 SecretRef；对象存储 S3 凭据、mail password、notification slack/webhook token 已确认消费点晚于 vault 并已接入配置侧 SecretRef。
>
> 额外观察（来自当前代码）：
> - `Storage::new`（`src/jupiter/storage/mod.rs:195`）仍保留为“自建 DB 连接 + object store”的便捷入口，但 `AppContext::new` 使用 `Storage::new_with_connection` 以复用 DB 连接并支持 vault 后解析对象存储 SecretRef。
> - `crate::jupiter::storage::object_storage::build_object_storage` 在 vault 就绪后被调用，因此 `object_storage.s3.access_key_id` / `secret_access_key` 的 `vault://` SecretRef 在对象存储构造前已解析为字面量；字面量凭据原样透传，env/IAM 部署不受影响。
> - `context/mod.rs:88` 的 `init_connection(&config.redis)` 在 Storage 之后、mail/notification 之前执行；失败会经 `AppContext::new` 返回错误。
...
- **配置侧 SecretRef 已有首批落点。** 现有直接消费者包括 `ssh_server_key`、PGP、Nostr、PKI，以及首批配置 SecretRef：`mail.password_ref` → `secret/config/...`；**`redis.url` 已支持 `vault://` SecretRef（2026-06-28）**，并在启动路径、validate、CLI 和 `--resolve-secrets` 中对齐；对象存储 `object_storage.s3.access_key_id` / `secret_access_key` 也已支持 `vault://` SecretRef，并在 validate、CLI 和 `--resolve-secrets` 中对齐。`database` 仍属于引导依赖，不能使用本项目 vault 的 SecretRef。
...
这些安全/引导事实直接约束了下文方案的边界：配置模块可以继续基于已完成的 vault/bootstrap/SecretRef 能力推进；数据库凭据仍不得改成本项目 vault SecretRef，而 Redis URL、对象存储凭据在初始化顺序重构（DB-only vault bootstrap，阶段 6/G）后已可在启动路径与 validate/CLI 路径完整解析 `vault://` SecretRef。
...
2. **区分“引导配置 / 早期运行时依赖”与“可迁移凭据”。** 数据库连接（以及 vault 自身启动所依赖的一切）属于**引导配置**，必须留在 TOML/环境变量中（明文或由 env 注入），**永远不能成为 vault SecretRef**——因为 vault 存在数据库里，连库才能起 vault。Redis URL 原属早期运行时依赖，初始化顺序已重构（DB-only vault bootstrap，阶段 6/G），**`redis.url` 已支持 `vault://` SecretRef（2026-06-28）**，启动路径、`config validate`、`config secret set/check`、`config validate --resolve-secrets` 均已对齐。对象存储凭据同理。只有在 vault 就绪后才被使用的凭据（例如邮件发送器密码、notification slack/webhook 凭据、Redis URL、对象存储 S3 凭据，以及未来新增且确认为后置消费的 OAuth/第三方服务 secret）才是“可迁移凭据”。
...
- `expand.rs`：集中处理占位符展开，定义支持哪些占位符、展开顺序、未知占位符的处理策略，以及是否允许递归展开；同时明确占位符只作用于字符串值，新增占位符前必须确认字段类型和替换顺序。当前支持两类占位符：`${var}`（config 内部变量引用，经 `envsubst`，自上而下、不递归展开替换结果）与 `${file:/path}`（文件挂载 secret，读取文件内容、去末尾换行、内容按字面量使用、**完全不进 `envsubst`**；禁止与 `${var}` 在同一值中混用，未闭合 `${file:` 报错，二者均由 `expand_value` 拒绝）。`variable_placeholder_substitute` 的原 `.unwrap()` panic 已收敛为 `ConfigError`，未解析/非法占位符值与文件读取失败已输出字段级脱敏诊断；后续仍需补完整跨 source 关系。
...
- `testing.rs`：首批已提供面向自动化测试的 `TestConfigBuilder`、`isolated_config()` 和 `TestSecretResolver`，可派生隔离的 base/cache/LFS/object storage 路径，接收 `.env.test` 风格的 DB/Redis/mail SecretRef 覆盖，并用内存 resolver 覆盖 secret 读取/缺失/evict 场景；`commands::config` 中涉及 validate/SecretRef 的命令层测试已开始使用该 helper 构造隔离配置。后续应继续把文件加载链路、Profile 合并和 CI 配置矩阵迁移到该 helper，避免测试直接复用或修改仓库中的基础配置文件。
- `validate.rs`：提供集中校验入口，按领域拆分校验函数，例如数据库连接串、连接池参数、监听端口、对象存储后端、路径可用性等，避免非法配置延迟到后续初始化阶段才暴露。首批 hard error 已覆盖**当前已存在字段**的无争议规则，例如 `database.db_type` 必须为 `postgres`、`database.db_url` scheme 必须为 `postgres`/`postgresql`、`database.max_connection > 0`、`database.min_connection <= database.max_connection`、数据库 acquire/connect timeout 非 0、`monorepo.import_dir`/`root_dirs`/`admin` 空值、`monorepo.rename.similarity_threshold` 范围、`log.level` 枚举、`lfs` 路径/URL、`build.orion_server`、`redis.url` scheme、Buck 并发/大小限制与 cleanup interval（吸收现有 `BuckConfig::validate()`）、object storage local/S3/S3-compatible/GCS 后端必填项、可选 `orion_server` 端口/URL/DB URL、`sidebar.default_items` 关键字段/重复 public_id，以及 mail 的 `mail.enabled = true` 时 `smtp_host`/`from` 必填、`password`/`password_ref` 互斥；首批 raw TOML warning 已覆盖 `[oauth]`、`[mail].smtp_tls`/`[mail].tls` 和任意未知字段。剩余 diagnostics 重点是环境变量/profile 来源、脱敏原始值和修复建议。但涉及尚不存在字段的校验（OAuth 回调地址等）仍应等 `OAuthConfig` 真实落地后再写。
- `reload.rs`：首批已提供 `ConfigHandle`，负责维护 `Arc<Config>` 快照，`reload_from_path` 复用 base/profile 加载流水线构建候选配置，对候选配置复用 `Config::validate()`，按白名单应用日志字段变更、邮件 dispatcher 关停变更、已运行 Buck cleanup 调度变更和已运行 artifact GC 调度变更，按白名单还应用邮件 provider/SMTP/`username`/`password`/`password_ref`/`from`/`starttls`/模板字段（经 mailer 异步重建与模板 registry 同步重建热生效），报告数据库、Redis、Buck 上传限制、artifact GC 从关闭到启用以及邮件重启类字段（`[mail]` 段增删）需重启，并在校验失败时保留旧配置与旧 mailer；`mail.password_ref` 等 SecretRef 变更经 mailer 重建热加载且不泄露引用；`ConfigReloadSubscriber` 已提供订阅组件应用和失败逆序回滚语义；`ConfigReloadWatcher` 已提供 base/profile 文件签名轮询、变化后触发 reload、失败记录并继续运行的执行层；`service` 命令已接入 watcher 生命周期并注册日志 reload subscriber，`AppContext` 已在邮件 dispatcher 启动时注册关停/限流/retry 与 mailer 重建/模板重建 subscriber，HTTP service 已在 Buck cleanup 和 artifact GC 启动时注册调度 subscriber。后续继续补更多真实消费端订阅接入。
...
2. **早期运行时依赖。** 这类字段不是数据库引导项，但在 vault 就绪前或同一初始化阶段已经被消费。**（2026-06-27 更新）** 初始化顺序已重构：`AppContext::new` 先做 DB-only `VaultCore` bootstrap，再 `resolve_object_storage_secrets` 解析对象存储 `vault://` SecretRef，最后构造完整 Storage，因此 `object_storage.s3.access_key_id`/`secret_access_key` 已可在启动路径走 SecretRef（字面量原样透传）；`config validate`、`config secret set/check` 和 `config validate --resolve-secrets` 现已接受并解析合法 namespace 下的对象存储 SecretRef（2026-06-28）。**（2026-06-28 更新）** `AppContext::new` 在对象存储之后才连接 Redis，且 `resolve_redis_url_secret` 会在 vault 就绪后解析 `redis.url` 中的 `vault://` SecretRef（合法 namespace 为 `vault://secret/config/<profile>/redis/url#<field>`），字面量 URL 原样透传；`config validate`、`config secret set/check` 和 `config validate --resolve-secrets` 现已对齐。
...
| `redis.url`（若含密码） | `AppContext::new` 中 vault 后 `resolve_redis_url_secret` → `init_connection` | 早期运行时依赖（已支持 vault SecretRef） | 已支持 `vault://secret/config/<profile>/redis/url#<field>`；`config validate`/`config secret set/check`/`--resolve-secrets` 已对齐；字面量 URL 原样透传；所有日志与错误必须脱敏 |
| `object_storage.s3.access_key_id` / `secret_access_key` | `AppContext::new` 中 DB-only vault bootstrap 后 `resolve_object_storage_secrets` → `build_object_storage` | 早期运行时依赖（已支持 vault SecretRef） | 已支持 `vault://secret/config/<profile>/object_storage/access_key_id#<field>` 与 `.../secret_access_key#<field>`；`config validate`/`config secret set/check`/`--resolve-secrets` 已对齐；字面量凭据原样透传 |
| `orion_server.db_url` 等 | Orion 作为独立服务使用 | 外部服务配置 | 由 Orion 自己或部署平台管理，monoengine 不应声称代管 |
| `mail.password_ref`（推荐）/ `mail.password`（兼容期明文） | `AppContext::new` 中 `VaultCore::new` 之后仅为 SMTP provider 解析，再构造 provider mailer 和 `EmailDispatcher` | **可迁移凭据（已落地首个成员）** | `provider`、`password_ref`、最小 resolver、互斥校验、mailer 错误传播、明文 deprecation warning、`SecretString` 防误打印、source diagnostics、`--deny-warnings` 门禁、失败发送可配置 dead-letter/retry、可配置有界并发、stale `sending` 恢复和作业管理 API 基线已落地；剩余=兼容期收尾治理、真实 SMTP/Mailpit、多实例矩阵和更完整 metrics/lifecycle |
...
- 互斥校验已存在：两者同时存在时为 hard error；两者都不存在时按原语义处理。集中 `validate.rs` 已覆盖互斥校验；source diagnostics 已覆盖 `mail.password` 明文来源 warning，且 warning 不输出原始值。
...
- **`mail.password` 仍是兼容期入口，但已改为 `Option<SecretString>`**：`Debug` 与 `Serialize` 输出已脱敏，明文只在 SMTP 适配层显式暴露；source diagnostics 已对 TOML/profile/env 来源输出不含值的 deprecation warning，仍需继续避免外部库边界回显真实密码；
...
**缓解措施**：
- 生产环境优先使用文件挂载 secret（如 K8s Secret volume、systemd `LoadCredential=`），再通过配置占位符 `${file:/path/to/secret}` 读取。**已落地（2026-06-27）**：`src/config/expand.rs` 支持 `${file:/path}` 占位符，读取所指文件内容作为该配置字段的值并去除单个末尾换行（`\n` 或 `\r\n`）。语义与约束：
  - **该字段值按字面量使用、绝不被再次当作模板展开**（即 secret 可安全包含 `$` / `${...}`）。
  - **不允许把 `${file:...}` 与其他 `${var}` 混在同一个值里**（如 `${file:/p}-${base_dir}`），也**不允许在文件路径里嵌套 `${...}`**（如 `${file:${base_dir}/secret}`）——这两种情况都会被拒绝（`FilePlaceholderMixed`）：secret 文件内容永不进入 `envsubst`，文件路径也不做变量展开；请让 `${file:/literal/path}` 作为某个专用字段的完整值、路径用字面量绝对路径。
  - 未闭合的 `${file:`（缺 `}`）会被拒绝（`FilePlaceholderUnterminated`，路径已脱敏）。
  - 读取失败返回脱敏诊断错误（含字段路径与文件路径，但**绝不**包含文件内容）。
  - **文件挂载 secret 永不作为 `${var}` 替换源**（`expand_value` 返回 `from_file` 标记，调用方不把该值写入替换变量表）：因此 secret 字节绝不会进入 `envsubst`，且若另一个字段用 `${该字段}` 去引用某个 `${file:...}` 字段，会**解析失败（fail-closed，`PlaceholderUnresolved`）而不是泄露 secret**。请把 `${file:...}` secret 放在专用字段中（如 `mail.password`），不要在别处引用。
  - 文件路径取自**受信任的**运维配置（可为绝对或相对路径，不做 base 目录沙箱限制，与 `base_dir`、对象存储根等其它配置路径同信任级别），且永不做变量展开；仅去除路径首尾空白作为便利（`${file: /p }` 可用），文件**内容**仍按字面量使用（只去掉单个末尾换行）。
  - 环境变量注入仍是常见选择，二者可按部署形态择一。
...
- **其他引导/早期凭据**：Redis URL、对象存储 key 等同样不在 `config init` 结果中预设真实凭据；只保留空字符串、示例占位符或部署平台 secret 注入说明。对象存储已支持启动路径与 validate/CLI 路径的 `vault://` SecretRef（阶段 6/G，2026-06-28 对齐），但 `config init` 默认不为对象存储生成 SecretRef 占位，以免把 bootstrap 类依赖默认为 vault。需要时运维人员可手动写入 `vault://secret/config/<profile>/object_storage/access_key_id#<field>` 与 `.../secret_access_key#<field>`。只有确认晚于 vault 消费、已真实接入 `Config` 且经校验支持的字段（例如已落地的 `mail.password_ref`）才在 `config init` 中生成 `SecretRef` 占位符。
...
Config::new                                  # context/mod.rs:30 起 AppContext::new
  ...
  -> validate mail provider/password refs    # vault 之后，检查 provider 与互斥关系
  -> resolve smtp mail.password_ref          # 通过 VaultSecretResolver 读取真实值
  -> mailer_from_config(...)                 # 构造 SMTP/console mailer，失败返回 MegaError
...
为降低配置初始化和敏感数据写入的使用门槛，顶层 `config` 子命令族已经部分落地，入口在 `src/commands/config.rs` 并已注册到 `src/commands/mod.rs`。当前可用能力包括 `config init`、`config validate`、`config validate --resolve-secrets`、`config secret ref`、`config secret set`、`config secret check`、`config secret rotate`、`config vault reset/rekey/backup/restore`；尚缺完整 source diagnostics、Profile 与样例/模板治理。
...
monoengine config validate --show-sources          # 已实现，显式打印 base/profile/env warning、字段来源图与覆盖关系
...
> 注意：下面的 `[mail]` 示例对应 `MailConfig` 的字段形态（扁平结构：`enabled`/`provider`/`smtp_host`/`smtp_port`/`username`/`password`/`password_ref`/`from`/`starttls`/`dispatcher_batch_size`/`dispatcher_max_in_flight`/`retry_max_attempts`/`retry_backoff_base_secs`/`retry_backoff_max_secs`/`attachment_prune_enabled`/`attachment_prune_interval_secs`/`attachment_retention_days`/`attachment_prune_statuses`）。**该结构现已作为 `MailConfig` 接入 `Config`**，且 `password_ref` 已可由 resolver 解析，因此 `config init` 可以直接生成 `password_ref` 占位。两点提示：(1) 仓库样例用 `provider = "smtp"`、`starttls = false`、端口 `2525` 关闭了 STARTTLS，仅适合本地开发，`config init` 生成生产骨架时应使用安全默认；(2) 样例里的 `smtp_tls`/`tls` 是 `MailConfig` 不识别的字段，会被 serde 丢弃，不应写入生成结果。
...
如果后续新增 OAuth client secret schema，应在对应模型真实存在、消费点依赖顺序确认后再给出示例。不要在文档中提前使用当前代码不存在的 `[oauth.github]` 结构。对象存储 SecretRef 的启动路径与 validate/CLI 路径已对齐（阶段 6/G，2026-06-28），示例中可使用 `vault://secret/config/<profile>/object_storage/access_key_id#<field>` 与 `.../secret_access_key#<field>`。
...
对于依赖敏感凭据的测试，不应在测试 TOML、`.env.test` 或日志中写入真实 API Key。配置模块应提供可注入的测试 secret resolver 或 vault 测试后端（现有 vault 测试已使用 `tempfile` + `test_storage` 构造隔离实例，可复用此模式），用固定的假 secret、过期 secret、缺失 secret 和权限失败场景覆盖消费端行为；缺失 `mail.password_ref` 与测试 resolver 权限拒绝已由命令层可注入 resolver 单测覆盖，真实 Vault resolver 已覆盖 secret 缺失、字段缺失和字段非字符串且不泄露 SecretRef 路径/字段/值，VaultCore ACL 单测已覆盖 config/generic runtime token 隔离。热加载相关测试也应使用测试配置文件和临时 resolver：分别验证基础字段热更新成功、不可热更新字段只告警不生效、SecretRef 变更遵循白名单、候选配置校验失败时旧配置继续生效。
...
还应补充配置兼容性测试矩阵：坏 TOML、坏环境变量类型、未知占位符、profile 类型冲突、数组覆盖语义、未知/未消费顶层段告警、SecretRef 路径缺少 `#field`、vault key 缺失、secret 缺失、secret 字段缺失、secret 字段非字符串、权限失败、错误信息脱敏、非 PostgreSQL 数据库配置被拒绝启动。`password` 与 `password_ref` 同时存在这类互斥测试，应在对应字段真实接入后再加入；其中坏 TOML 源行脱敏 smoke、secret 缺失/字段缺失/字段非字符串的真实 Vault resolver 脱敏单测、测试 resolver/命令层消费端权限失败脱敏单测，以及 config/generic runtime token ACL 隔离单测已落地。
...
热加载应复用同一条解析、展开、规范化和校验流水线，避免启动加载与运行期重载出现两套语义。`reload.rs` 首批已维护 `ConfigHandle`，内部持有当前生效的 `Arc<Config>`，并支持从 base/profile 路径构建候选配置、候选配置校验、日志字段白名单更新、`mail.enabled` 从 true 到 false 的运行期关停、`mail.dispatcher_batch_size` / `mail.dispatcher_max_in_flight` 运行期限流更新、`mail.retry_max_attempts` / `mail.retry_backoff_base_secs` / `mail.retry_backoff_max_secs` 运行期 retry policy 更新、`mail.attachment_prune_*` 运行期附件保留策略更新、已运行 Buck cleanup 与 artifact GC 任务的调度参数热更新与关停、邮件 provider/SMTP/`username`/`password`/`password_ref`/`from`/`starttls`/模板字段经 mailer 异步重建与模板 registry 同步重建热更新、数据库/Redis/Buck 上传限制/邮件重启类字段（整体新增移除）需重启报告、`mail.password_ref` 等 SecretRef 变更不泄露引用、失败保留旧快照，以及订阅组件应用/失败回滚；`ConfigReloadWatcher` 已可轮询 base/profile 文件签名变化并触发同一 reload 流水线，坏配置会被拒绝并继续运行；`service` 命令已在服务生命周期内启动 watcher，并在子命令返回后关闭 watcher；日志 reload subscriber 已注册到 service 的 `ConfigHandle`，使 `log.level` / `log.print_std` / `log.with_ansi` 变化先应用到 tracing layer 再发布新快照；邮件 dispatcher subscriber 已在 dispatcher 启动时注册，使 `mail.enabled = false` 停止处理待发送队列，并使 dispatcher 限流、retry policy 与附件保留策略字段热更新；Buck cleanup 与 artifact GC subscriber 已在 HTTP service 启动对应任务时注册，使任务调度字段和 `enable=false` 作用于运行中任务；`Storage`/`AppContext` 已接入共享 `ConfigHandle`，业务组件经 `storage.config()` 可读取当前稳定快照。当配置文件或受支持的配置源发生变化时，先构建候选 `Config`，再执行差异计算和白名单校验。日志级别、日志输出细节、部分功能开关、任务调度间隔、邮件通知关停、dispatcher 限流、retry policy、附件保留策略，以及邮件 provider/SMTP 参数/`username`/`password`/`password_ref`/`from`/`starttls`/模板字段（经 mailer 异步重建与模板 registry 同步重建热生效、失败保留旧值，不在主流程同步重建长生命周期连接）等字段可以直接热更新；数据库、Redis、对象存储、监听地址端口、OAuth 客户端密钥、Buck cleanup/artifact GC 从关闭到启用、整体新增/移除 `[mail]` 段等字段默认视为启动期配置，检测到变化时应记录告警并提示重启，而不是在运行期隐式重建。
...
- 对象存储凭据**（2026-06-27）已可走 vault SecretRef，（2026-06-28）validate/CLI 已对齐**：`AppContext::new` 经 DB-only vault bootstrap + `resolve_object_storage_secrets` 在建对象存储前解析 `object_storage.s3.access_key_id`/`secret_access_key` 中的 `vault://` SecretRef（字面量原样透传）；`config validate` 接受合法 namespace 的 object_storage SecretRef，`config secret set/check` 支持写入，`config validate --resolve-secrets` 可解析验证。详见 vault.md 阶段 G / config.md 阶段 6。
...
8. 已部分完成：新增 `error.rs`，并把 `variable_placeholder_substitute` 原 10 处 `unwrap` 替换为 `ConfigError` 诊断；未解析/非法占位符值已脱敏并带修复建议；`mega_base`/`mega_cache` 的早期目录解析 `unwrap` 已改为 env/system/fallback 的无 panic 路径；`database_connection` 的连接和 migration `expect`、Redis 初始化的 `expect`/`panic`、BuckService 构造 `expect`、HTTP 监听地址解析/绑定 `unwrap`、`config secret` 命令执行期 clap 必填参数 `expect` 已改为返回错误并由启动/bootstrap/命令调用方传播。剩余加载路径上的 `expect`/`panic` 仍需继续收敛，这会改变失败形态，不应并入纯移动阶段。
...
10. 已完成首批：`mail.password` 明文兼容路径改为 `Option<SecretString>`，Debug/Serialize 输出脱敏；`config validate` 与服务启动路径会对明文 `mail.password` 输出 deprecation warning；TOML/profile/env 来源已进入 `ConfigSourceDiagnostics`，`--deny-warnings` 和 CI smoke 可阻断该兼容期字段；`.expose_secret()` 集中在 `SmtpMailer::new` 适配层。后续按兼容期策略继续推动样例配置和生产配置迁移到 `mail.password_ref`。
11. 已完成首批：新增 `src/config/validate.rs`，实现 `Config::validate()` 首批 hard error 校验，覆盖 `database.db_type`、`database.db_url` scheme、数据库连接池参数、`monorepo.import_dir`/`root_dirs`/`admin` 空值、`monorepo.rename.similarity_threshold` 范围、`pack` 大小/缓存路径/通道容量、`blame` 阈值、`log.level`、`lfs` 路径/URL、`build.orion_server`、`redis.url`、`mail.provider`、`mail.password` / `mail.password_ref` 互斥、`mail.password_ref` namespace（只允许 `vault://secret/config/<profile>/mail/password#<field>`）、SMTP provider 下 `mail.enabled` 时 `smtp_host` / `from` 必填、`mail.dispatcher_batch_size` / `mail.dispatcher_max_in_flight` 非零、`mail.retry_max_attempts` / `mail.retry_backoff_base_secs` / `mail.retry_backoff_max_secs` 非零且 backoff 上限不小于基础值、`mail.template_default_locale` 非空且字符合法、已配置的 `mail.template_dir` 必须指向既有目录、`mail.attachment_prune_*` 非零且只允许清理 `sent` / `skipped` 终态附件、Buck 限制、artifact GC 调度参数、object storage local/S3/S3-compatible/GCS 后端必填项、`sidebar.default_items` 关键字段/重复 public_id，以及可选 `orion_server` 端口/URL/DB URL；`config validate` 已复用该入口，`Storage::new` 的 Buck 非法配置已改为返回 `MegaError`。后续集中校验主要随新增字段继续补规则。
12. 已完成首批：`config validate` 读取原始 TOML，对孤立 `[oauth]` 顶层段、`[mail].smtp_tls` / `[mail].tls`、兼容期 `[mail].password` 以及任意未知字段输出 warning，并覆盖嵌套 table 与数组内 inline table；文件级 warning 已通过 `known_unconsumed_file_fields` 保留 source path，可测试 base/profile 来源；同时会扫描 `MEGA_*` 环境变量名，对未知覆盖项、`MEGA_OAUTH__...`、`MEGA_MAIL__TLS` / `MEGA_MAIL__SMTP_TLS` 和 `MEGA_MAIL__PASSWORD` 输出不含变量值的 warning，并忽略 `MEGA_CONFIG`、`MEGA_PROFILE`、`MEGA_BASE_DIR`、`MEGA_CACHE_DIR` 等加载器运行时变量。base/profile/env warning 已由 `ConfigSourceDiagnostics` 汇总，`config validate --deny-warnings` 可将这些 warning 升级为失败，供 CI 严格门禁使用；`config validate --show-sources` 已能显式输出 base/profile/env warning、字段来源图以及 profile/env 覆盖 base/profile 的字段路径和来源，不输出原始值，并对覆盖关系与字段来源输出 source-level 修复建议；对 `database.db_url`、`redis.url`、object storage key、`mail.password(_ref)` 等敏感字段，source field/source override 会明确提示敏感值已省略且真实凭据应走部署/env secret 或受支持 SecretRef；对 `monorepo.root_dirs` 等数组字段覆盖，会提示数组整体替换而不是追加。后续仍需扩展为更完整 source diagnostics，覆盖 profile 诊断和更多坏输入场景。
...
16. 已完成首批：`config init` 生成安全默认骨架，不写入可预测生产密码；数据库、Redis 凭据只给 env/部署 secret 指引；对象存储 S3 凭据已支持 `vault://` SecretRef，但 `config init` 默认不为其生成 SecretRef 占位（避免把 bootstrap 类依赖默认为 vault），需要时运维人员可手动写入 `vault://secret/config/<profile>/object_storage/access_key_id#<field>` 与 `.../secret_access_key#<field>`；对 `mail.password_ref` 生成 SecretRef 占位，并提示后续使用 `config secret set --value-stdin` 写入真实值。
...
18. 保持已实现的 `config secret ref/set/check` 行为：允许后置可迁移凭据（`mail.password`、`redis.url`、`notification.slack.webhook_url`、`notification.webhook.token`、`object_storage.s3.access_key_id`、`object_storage.s3.secret_access_key`，见 `SUPPORTED_SECRET_FIELDS`），继续拒绝 database 引导凭据；写入/检查路径必须位于受控 `config/<profile>/...` namespace（如 `config/<profile>/mail/password`、`config/<profile>/redis/url` 或 `config/<profile>/object_storage/access_key_id`），错误诊断不回显实际 vault path。
...
20. 已完成首批：固定 Profile 文件命名、加载优先级、数组覆盖语义和 SecretRef namespace，并补充 profile 合并、profile/env 覆盖顺序和最小 vault bootstrap 合并测试；完整 profile source diagnostics（base/profile/env 矩阵）已落地（2026-06-27）；后续仅 CI 矩阵与更多坏输入场景为增量补充。
21. 已完成首批并继续推进：新增 `testing.rs`，提供 `TestConfigBuilder`、`isolated_config()`、`.env.test` 风格覆盖合并和 `TestSecretResolver`；CLI `parse` 的无子命令加载单测、`commands::config` 中涉及 validate/SecretRef 的命令层测试、`config::validate` 校验矩阵、`Storage::mock()` 和 `jupiter::tests::test_storage()` 已迁到隔离配置 helper；`Config::mock()` 已收窄为 test-only helper，当前没有运行期调用方。`EnvVarGuard::remove()` 与中毒可恢复的 `env_lock()` 已落地，使文件/Profile 优先级单测（`test_vault_bootstrap_loads_profile_database_override`、`reload_from_path_uses_profile_candidate_and_keeps_restart_required_fields`）在 `.env.test` 注入 `MEGA_*` 的情况下仍能稳定通过，单点 panic 不再级联成 `PoisonError`。后续继续把更多业务测试迁移到该 helper。
22. 已完成首批：新增 `.github/workflows/config-validation.yml`，在 CI 中覆盖基础样例、默认模板、`config init` 结果、profile 合并结果、`MEGA_*` 未消费覆盖项单测、坏 TOML 源行脱敏 smoke、坏 env/profile 类型脱敏单测、monorepo/pack/blame/sidebar/artifact GC 坏参数单测、object storage SecretRef namespace 校验与 `--resolve-secrets` 解析单测、缺失 `mail.password_ref` 与测试 resolver 权限拒绝的命令层脱敏单测、`mail.password_ref` 错误 namespace 脱敏单测与 CLI smoke、source warning/field/override 脱敏与修复建议单测、敏感覆盖脱敏和数组替换提示 smoke、热加载静态消费端 restart-required 矩阵，以及坏占位符、坏 SecretRef URI、坏 Redis URL scheme、兼容期 `mail.password` 和 profile 废弃字段的 CLI 失败 smoke；真实 Vault resolver 已覆盖 secret 缺失、字段缺失和字段非字符串脱敏单测，VaultCore ACL 单测已覆盖 config/generic runtime token 隔离。后续继续把 `.env.test` 隔离测试配置和更多坏输入矩阵纳入 CI。
...
25. ✅ **已落地（2026-06-28）**：`redis.url` 现可配置为 `vault://` SecretRef，由 `AppContext::new` 中的 `resolve_redis_url_secret` 在 vault 就绪后解析；`config validate`/`config secret set/check`/`--resolve-secrets` 已对齐合法 namespace `vault://secret/config/<profile>/redis/url#<field>`。其它早期字段（Orion 等）暂未迁移为 SecretRef；如需迁移，沿用同一 post-vault 解析模式并确认失败语义可接受。
...
26. 已完成首批：新增 `reload.rs`，提供 `ConfigHandle` 快照句柄，`reload_from_path` 复用 base/profile 文件加载流水线构建候选配置，复用 `Config::validate()` 校验候选配置，按字段白名单应用 `log.level` / `log.print_std` / `log.with_ansi`、`mail.enabled` true→false 关停、`mail.dispatcher_batch_size` / `mail.dispatcher_max_in_flight` 运行期热更新、`mail.retry_max_attempts` / `mail.retry_backoff_base_secs` / `mail.retry_backoff_max_secs` 运行期热更新、`mail.attachment_prune_*` 运行期热更新、已运行 Buck cleanup 的 `enable_session_cleanup` true→false、`cleanup_interval`、`completed_retention_days` 变更，以及已运行 artifact GC 的 `enable` true→false、`interval_secs`、`grace_secs`、`batch_limit` 变更，以及邮件 `provider`/`smtp_host`/`smtp_port`/`username`/`password`/`password_ref`/`from`/`starttls`/`template_default_locale`/`template_dir` 变更（经 mailer 异步重建与模板 registry 同步重建热生效、失败保留旧值）；数据库、Redis、Buck 上传限制、Buck cleanup/artifact GC 从关闭到启用、整体新增/移除 `[mail]` 段等字段变化报告需重启；base_dir、monorepo、pack、lfs、blame、build、object_storage、orion_server、sidebar 等静态消费端字段已保守报告需重启且不发布候选快照，并覆盖对象存储密钥不泄露；失败时保留旧配置与 mailer，`mail.password_ref` 等 SecretRef 变更不泄露引用；`Storage`/`AppContext` 已持有共享 `ConfigHandle`，`storage.config()` 已从 handle 读取当前快照；`ConfigReloadSubscriber` 已支持订阅组件在发布前应用变更、任一失败时逆序回滚并保留旧快照；`ConfigReloadWatcher` 已支持轮询 base/profile 文件签名变化并触发 reload，坏配置被拒绝且进程可继续运行；`service` 命令已在服务生命周期内接入 watcher。
...
28. 改造真实消费端接入订阅方式（日志、功能开关、任务调度、邮件通知等只订阅各自可热更新字段；不可热更新字段变化只告警提示重启）。日志 reload subscriber 已首批注册到 service 启动路径；邮件 dispatcher 已在启动时注册 `mail.enabled = false` 的关停、dispatcher batch / max-in-flight 更新和 retry policy 更新 subscriber，邮件 mailer 重建 subscriber（provider/SMTP/凭据变更后异步重建 SMTP mailer，vault 后重新解析 `password_ref`，失败保留旧 mailer）与模板 registry 重建 subscriber 也已注册；HTTP Buck cleanup 与 artifact GC 已在任务启动时注册调度 subscriber。仅整体新增/移除 `[mail]` 段继续明确为重启字段（`mail.enabled` true↔false 已运行期热加载）。
29. 已完成首批核心测试：可热更新日志字段生效、base/profile 文件候选可加载、数据库字段变化只报告需重启且不发布、base_dir/monorepo/pack/lfs/blame/build/object_storage/orion_server/sidebar 静态消费端字段只报告需重启且不发布/不泄露对象存储密钥、SecretRef 变更遵循白名单且不泄露引用、候选校验失败保留旧快照、订阅组件发布前应用、订阅组件应用失败时回滚并保留旧快照、profile 文件变化触发 watcher reload、非法 profile 变化保留旧快照、watcher shutdown 正常退出、service watcher task 触发 profile reload、service 注册日志 reload subscriber 后可发布日志字段变更、`mail.enabled` true→false 可发布且 dispatcher 停止处理 pending job、`mail.dispatcher_batch_size` / `mail.dispatcher_max_in_flight` 可发布且 dispatcher control 更新、`mail.retry_max_attempts` / `mail.retry_backoff_base_secs` / `mail.retry_backoff_max_secs` 可发布且 dispatcher retry policy 更新、`mail.attachment_prune_*` 可发布且 dispatcher control 更新、`mail.enabled` true↔false 可发布快照且 dispatcher 按新值启停（`reload_applies_mail_enable_without_restart`）、邮件 provider 变更可发布快照（`reload_applies_mail_provider_change_and_publishes_snapshot`）、模板设置变更可发布快照（`reload_applies_mail_template_settings_and_publishes_snapshot`）、provider/SMTP 参数/用户名/明文兼容密码/from/starttls 整体重配可发布快照且不泄露明文或 SecretRef 路径（`reload_applies_mail_reconfiguration_and_publishes_snapshot_without_leaking_secrets`）、`mail.password_ref` 变更可发布且不泄露（`reload_applies_secret_ref_change_and_publishes_without_leaking`）、Buck cleanup 运行期调度字段可发布且 subscriber 更新 control、Buck cleanup 从关闭到启用仍报告需重启、Buck 上传限制字段只报告需重启、artifact GC 运行期调度字段可发布且 subscriber 更新 control、artifact GC 从关闭到启用仍报告需重启。后续继续补更多真实消费端订阅测试。
...
| **6** | 对象存储后置初始化 | 已落地：DB-only vault bootstrap 后解析 `object_storage.s3` 凭据的 `vault://` SecretRef | 无 |
...
3. 对象存储凭据的启动路径、`config validate`、`config secret set/check`、`config validate --resolve-secrets` 的 SecretRef 解析与 namespace 校验已全部对齐（DB-only vault bootstrap，阶段 6/G，2026-06-28）。
...
3. **确认敏感字段边界**
   - 当前经 `config validate`/`--resolve-secrets` 支持的配置侧 Vault SecretRef 包括 `mail.password_ref`、`redis.url`、`notification.slack.webhook_url_ref`/`notification.webhook.token_ref`，以及对象存储 `object_storage.s3.access_key_id`/`secret_access_key`（2026-06-28 已对齐 validate/CLI/`--resolve-secrets`）；`config secret set/check` 支持 `mail.password`、`redis.url`、`notification.slack.webhook_url`、`notification.webhook.token`、`object_storage.s3.access_key_id`、`object_storage.s3.secret_access_key`（见 `SUPPORTED_SECRET_FIELDS`）。`config validate --show-sources` 与 source diagnostics 已把上述 SecretRef 字段（含 `notification.*_ref`）作为敏感字段处理，诊断输出中不回显其实际 Vault 路径。
   - database 凭据继续走 TOML/env/部署平台 secret；**Redis URL 已支持 `vault://` SecretRef，合法 namespace 为 `vault://secret/config/<profile>/redis/url#<field>`，字面量 URL 原样透传**；对象存储 S3 凭据已支持 `vault://` SecretRef，合法 namespace 为 `vault://secret/config/<profile>/object_storage/access_key_id#<field>` 与 `.../object_storage/secret_access_key#<field>`，字面量凭据原样透传。
   - 对象存储的初始化顺序重构（`AppContext::new` 的 DB-only vault bootstrap）已落地，启动路径、`config validate`、`config secret set/check`、`config validate --resolve-secrets` 均已支持 object_storage 的 `vault://` SecretRef。
...
3. 错误模型、脱敏与校验：占位符展开的原 `unwrap` 已收敛为 `ConfigError`，未解析/非法占位符值已脱敏；`mega_base` / `mega_cache` 的早期目录解析 `unwrap` 已移除；数据库连接/migration 初始化、Redis 初始化、BuckService 构造和 HTTP 监听地址解析/绑定已改为返回错误；首批 URL redaction、`SecretString`、`SecretRef` 默认输出脱敏、统一 `MEGA_*` source builder 和 `validate.rs` 已落地，并已继续覆盖 monorepo/pack/blame/sidebar/artifact GC 明显非法参数；继续收敛剩余加载路径 `expect`/`panic`，补更多配置规则和完整 source diagnostics。
4. 初始化与诊断：`config init` 首批已落地，已生成安全默认模板和 `mail.password_ref` 占位；`config validate` 已使用 `LoadMode::RawSources` 命令内解析，raw TOML、`MEGA_*` 未消费覆盖项 warning、`mail.password` 兼容期 source warning、文件级 source path 诊断、坏 env/profile 类型脱敏错误和首批修复建议已完成；继续补完整 RawSources/source diagnostics。
5. 样例/Profile/测试分层：测试配置生成器、Profile 合并语义、基础样例凭据治理和 CI 配置验证入口已完成首批；CI 已纳入 env diagnostics、坏 TOML 源行脱敏 smoke、坏 env/profile 类型单测、monorepo/pack/blame/sidebar/artifact GC 坏参数单测、缺失 `mail.password_ref` 命令层脱敏单测，以及坏占位符、坏 SecretRef URI、坏 Redis URL scheme 的 CLI 失败 smoke；真实 Vault resolver 已覆盖 secret 缺失、字段缺失和字段非字符串脱敏；继续扩展 CI 坏输入矩阵并迁移更多测试到隔离 helper。
...
- [x] 已完整阅读“事实校准” + “当前实现状态速览表” + “硬约束”三部分。
- [x] 已确认本次阶段**不会**尝试把数据库凭据做成 SecretRef；Redis URL 与 object storage 凭据已支持 `vault://` SecretRef。
- [x] 已确认本次阶段**不会**在 `Config::new` 内部调用 vault 或做异步 secret 解析。
- [x] 已确认不会重复实现 `LoadMode`、`config secret`、`SecretRef`、`mail.password_ref` 或 Vault fail-closed。
- [x] 如果涉及 `mail`，已把工作聚焦在明文兼容治理、SecretString/redaction、source diagnostics 或 dispatcher 生命周期。
...
- [x] 已在 `validate.rs`/`config validate` 中加入首批 `MEGA_*` 未消费/被忽略/废弃覆盖项告警，覆盖未知 env 覆盖项、`MEGA_OAUTH__...`、`MEGA_MAIL__TLS`/`MEGA_MAIL__SMTP_TLS`、`MEGA_MAIL__PASSWORD`，并避免输出 env 值。
...
- [x] 已提供 `config validate --show-sources` 显式输出 base/profile/env warning、字段来源图和覆盖关系，输出只包含来源、字段路径和修复建议，不包含配置值。
...
- [x] 已给 source field/source override 中的敏感字段补入 `sensitive values are omitted` 和部署/env secret/SecretRef 指引，继续避免输出原始值。
- [x] 已给数组字段 source override 补入整体替换而非追加的 profile 合并提示，继续避免输出数组原始值。
...
- [x] 已确认顶层移动与调用方路径迁移已完成；后续错误语义、Profile、热加载分别单独提交，`config init` 模板治理不与这些阶段混合。
- [x] 已在 CI 中增加配置样例首批校验任务（基础 + 生成 + profile + env diagnostics + 坏 TOML 源行脱敏 smoke + 坏 env/profile 类型单测 + monorepo/pack/blame/sidebar/artifact GC 坏参数单测 + 缺失 `mail.password_ref`/测试 resolver 权限拒绝命令层脱敏单测 + `mail.password_ref` 错误 namespace 脱敏单测/CLI smoke + source warning/field/override 脱敏和修复建议单测 + 真实 Vault resolver 的 secret 缺失/字段缺失/字段非字符串脱敏单测 + VaultCore config/generic runtime token ACL 隔离单测 + `mail.password` 兼容期 source warning 单测/CLI smoke + profile 废弃字段 `--show-sources`/`--deny-warnings` smoke + 坏占位符/坏 SecretRef URI/坏 Redis URL scheme CLI smoke）；更多坏输入和完整 source diagnostics 仍待补。
