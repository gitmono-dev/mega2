...
还应补充配置兼容性测试矩阵：坏 TOML、坏环境变量类型、未知占位符、profile 类型冲突、数组覆盖语义、未知/未消费顶层段告警、SecretRef 路径缺少 `#field`、vault key 缺失、secret 缺失、secret 字段缺失、secret 字段非字符串、权限失败、错误信息脱敏、非 PostgreSQL 数据库配置被拒绝启动。`password` 与 `password_ref` 同时存在这类互斥测试，应在对应字段真实接入后再加入；其中坏 TOML 源行脱敏 smoke、secret 缺失/字段缺失/字段非字符串的真实 Vault resolver 脱敏单测已落地，权限失败已由测试 resolver 和命令层消费端脱敏单测覆盖，后续继续补真实 Vault ACL 级权限失败。
