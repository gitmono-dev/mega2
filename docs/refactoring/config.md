...
- `reload.rs`：首批已提供 `ConfigHandle`，负责维护 `Arc<Config>` 快照，`reload_from_path` 复用 base/profile 加载流水线构建候选配置，对候选配置复用 `Config::validate()`，按白名单应用日志字段变更、邮件 dispatcher 关停变更、已运行 Buck cleanup 调度变更和已运行 artifact GC 调度变更，按白名单还应用邮件 provider/SMTP/`username`/`password`/`password_ref`/`from`/`starttls`/模板字段（经 mailer 异步重建与模板 registry 同步重建热生效），报告数据库、Redis、Buck 上传限制、artifact GC 从关闭到启用以及邮件重启类字段（`mail.enabled` false→true、`[mail]` 段增删）需重启，并在校验失败时保留旧配置与旧 mailer；`mail.password_ref` 等 SecretRef 变更经 mailer 重建热加载且不泄露引用；`ConfigReloadSubscriber` 已提供订阅组件应用和失败逆序回滚语义；`ConfigReloadWatcher` 已提供 base/profile 文件签名轮询、变化后触发 reload、失败记录并继续运行的执行层；`service` 命令已接入 watcher 生命周期并注册日志 reload subscriber，`AppContext` 已在邮件 dispatcher 启动时注册关停/限流/retry 与 mailer 重建/模板重建 subscriber，HTTP service 已在 Buck cleanup 和 artifact GC 启动时注册调度 subscriber。后续继续补更多真实消费端订阅接入。
