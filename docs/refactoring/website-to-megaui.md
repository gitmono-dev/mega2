...
| 项 | 原值 | 新值 |
|---|---|---|
| `website-next` / `website-db-init` `build.context` | `../monoui` | `../megaui` |
...
| 新增服务 | — | `megaui-collab`（host `17002` → `7002`） |
| `website-next` 注入 | — | `MEGA_COLLAB_*` / `NEXT_PUBLIC_MEGA_COLLAB_PUBLIC_URL` |
| bucket | 仍默认 `monoui`（可选改名未做，见 ADR-ARCH-05） | 不变 |

## 原因

megaui 取代 monoui 作为本联调栈的前端构建产物；collab 入仓后需独立
`megaui-collab` 服务供浏览器 WS 与内网 bridge 使用。
