...
| HTTP API | 部分实现 | `chat_router` 已挂载，DTO/OpenAPI 标注存在；**成员管理 HTTP 端点已暴露（owner 可添加/移除成员，成员可列成员，2026-06-29 落地）**；稳定无 Redis router 测试已落地；仍缺真实 HTTP 黑盒矩阵和更完整错误码兼容性。 |
...
| 集成测试 | 部分实现 | service/router 生命周期测试存在；router 生命周期测试已通过 `ChatApiState` 去除 Redis 依赖，不再在 Redis 不可用时 skip；仍需补真实 HTTP 黑盒矩阵。 |
