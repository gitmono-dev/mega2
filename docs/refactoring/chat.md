...
5. **仍未完成**：外部数据迁移工具的真实源库导入/校验、实时事件 broadcaster（当前 no-op）、完整真实 HTTP 黑盒矩阵和聊天提及/回复通知。message notification 内部状态已完成首批 reply 写入；mention 解析与外部通知投递仍属于后续。附件 presign/confirm 已有首批 file name/type/size/object-key 校验；后续仍需按产品策略补更完整 MIME allowlist 与存储对象归属校验。
...
| Channel Chat（频道、消息） | 部分实现 | channel/message/membership 实体、迁移、storage、service 已落地；create/send/edit/delete/read/unread/member service 主路径可用；reply message notification 内部状态已写入，mention notification 仍未实现。 |
...
- 根据 mention/reply 规则写 `message_notifications`，但不做外部投递；当前已完成 reply 首批：回复其他当前成员的消息时，为被回复消息的 sender 写一条内部提醒，self-reply 或 sender 已不在 channel 时不写。
