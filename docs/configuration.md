...
- **`[agent_capture]`**: `enabled` + `tenant_id` / `deployment_id` + `[[agent_capture.ingest_tokens]]` (name / token / paths). Storage-only only; enabling requires an explicit `git.push_auth` and at least one token. The Architecture Guide lists its route and gate.
