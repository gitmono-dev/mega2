...
- `IMPORT_REPO_REMOVED`：receive-pack 在 `unpack ok` 之后，每个 tag 各自得到 `ng refs/tags/<t> IMPORT_REPO_REMOVED: …`；分支批的每个分支得到同一原文的 `ng` 行，不论是哪一道栅栏拒绝（unpack 批、`file_path` 步骤、B3 attach，或已完成的仅删除轮次被重放）；git 客户端显示 `! [remote rejected] main -> main (IMPORT_REPO_REMOVED: …)`。`POST /api/v1/edit/save` 在对象行写入时遇到栅栏返回 409（`err_message` 为同一原文，零对象行）；更早的预读阶段遇到 detach 仍为既有的 500（`DEFER-FU-33`，FU-19）。写入队列行为 `Failed`，`failure_type` 为 `AttachFailure`，`error_message` 为同一原文。
