...
| [`plan-20260917.md`](plan-20260917.md) | 日期计划 | **执行中（LB-01 done；LB-02 done，v0.10.41；LB-03 done，v0.10.42；LB-04 done，v0.10.43；LB-05 done，v0.10.44；LB-06 done；LB-07 pending）**（补全 Libra `mega2 browser` 所需产品 HTTP：`POST /api/v1/delete-entry`、`POST /api/v1/move-entry`；把已有 `tag_router` 挂到 storage-only 并在 trunk 上对 create/delete 走 `push_auth`；契约正文将为 `docs/refactoring/directory-entry-api.md`。LB-01（契约页）、LB-02（`delete-entry`，v0.10.41）、LB-03（`move-entry`，v0.10.42）、LB-04（storage-only tags，v0.10.43）、LB-05（compose 黑盒 case，v0.10.44）与 LB-06（Libra pin 交接，纯文档）已 `done` / `complete`，LB-07 `pending`；中间态由各卡的收口提交刷新，最终态由 LB-07 登记。不改 `GET /tree` / `POST /create-entry` 字段；不把 list tags 改成 GET；get/delete 无 path 选择器（handler 固定以 `/` 分发，服务层按 tag 名全局查找）） |
