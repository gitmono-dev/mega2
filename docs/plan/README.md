...
| [`plan-20260912.md`](plan-20260912.md) | 日期计划 | **收口中（WH-15 remote-pending）**（storage-only 提交后出站 webhook：运维静态配置、committed-write emitter，有界、仅元数据、带 HMAC。**WH-01..WH-14 全部 done/complete；WH-15 已实现（v0.10.40 bump 提交），待推送、tag 与 `docker` job 绿灯后翻 done/complete**：`[storage_events]` 配置表面、HTTPS HMAC 运输、DNS 地址钉住、事件投影与静态过滤、有界 emitter 运行时、CLI/service 清理尾段、启动 secret 绑定，以及六类来源 hook（`repo.push` / `oci.manifest.published` / `lfs.object.uploaded` / `lfs.media.finalized` / `agent_capture.events.committed` / `agent_capture.checkpoint.committed`）。WH-07/WH-08 经 `DEP-WH-01` 依赖 [`plan-20260911.md`](plan-20260911.md) 的 AC-15，已满足。**收口中**：2026-09-16 完成度复核追加的 `WH-14`（`shutdown_grace_seconds` 范围门与 `oci_repositories` 规范名门，v0.10.39，done/complete，已于 2026-09-17 发布）与 `WH-15`（`installation_id` 记录字段与 drop 记账，v0.10.40 bump 提交，`remote-pending` 直至推送、tag 与 `docker` job 绿灯），契约正文 [`../refactoring/storage-events.md`](../refactoring/storage-events.md)。永久排除 webhook CRUD / 入站 webhook / outbox / 持久 delivery 状态 / retry worker；`DEFER-WH-01`（presigned 直传确认）与 `DEFER-WH-02`（media 静态 token 认证）仍延后） |
...
| [`plan-20260917.md`](plan-20260917.md) | 日期计划 | **新建（设计稿，0 实现）**（补全 Libra `mega2 browser` 所需产品 HTTP：`POST /api/v1/delete-entry`、`POST /api/v1/move-entry`；把已有 `tag_router` 挂到 storage-only 并在 trunk 上对 create/delete 走 `push_auth`；契约正文将为 `docs/refactoring/directory-entry-api.md`。LB-01..07 全部 `pending`。不改 `GET /tree` / `POST /create-entry` 字段；不把 list tags 改成 GET；get/delete 保持 root-only） |
