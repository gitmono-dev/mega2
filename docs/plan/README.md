...
| [`plan-20260917.md`](plan-20260917.md) | 日期计划 | **执行中（LB-01 done；LB-02 done，v0.10.41；LB-03 done，v0.10.42；LB-04..07 pending）**（补全 Libra `mega2 browser` 所需产品 HTTP：`POST /api/v1/delete-entry`、`POST /api/v1/move-entry`；把已有 `tag_router` 挂到 storage-only 并在 trunk 上对 create/delete 走 `push_auth`；契约正文将为 `docs/refactoring/directory-entry-api.md`。LB-01（契约页）、LB-02（`delete-entry`，v0.10.41）与 LB-03（`move-entry`，v0.10.42）已 `done` / `complete`，LB-04..07 `pending`；中间态由各卡的收口提交刷新，最终态由 LB-07 登记。不改 `GET /tree` / `POST /create-entry` 字段；不把 list tags 改成 GET；get/delete 保持 root-only） |
