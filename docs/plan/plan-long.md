...
| [`plan-20260907.md`](plan-20260907.md) | PT-03（hash 切片） | **执行中**（B3-01..B3-04 已推；B3-05 索引） | 钉 git-internal 0.9.0；显式 `HashKind`；blake3 bootstrap + Libra/git-internal wire/pack normal service；Git/LFS 独立 hash-domain（`Git=blake3/LFS=sha256` 与 `Git=sha256/LFS=blake3` 可表达）。不宣称标准 Git BLAKE3 互通（DEFER-B3-01）；不做原地 migration（DEFER-HSH-03）；LFS BLAKE3 业务面 `DEFER-B3-LFS-01`。 |
