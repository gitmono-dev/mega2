...
| [`plan-20260911.md`](plan-20260911.md) | N/A（storage-only Agent Capture；非 Mega PT） | **实现完成（待 AC-15 收口）** | libra 两套 session 以 raw 落入 `agent_capture_*` + `ObjectNamespace::Agent`；独立 ingest token；review 不挂载。不实现 libra 客户端（DEFER-AC-01）、不默认 FileHistory 50 版（DEFER-AC-02）。 |
