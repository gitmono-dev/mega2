...
| [`plan-20260901.md`](plan-20260901.md) | PT-03（FastCDC Media 切片）+ N/A（receive-pack/导入/DB/Redis/ID；不新增 PT/SB） | **已完成**（FC-14 `0.9.0`；独立卡至 **v0.10.0** / `634904b`） | `--features fastcdc` Media：`libra/media/v1`、scope、finalize+LFS fallback、Libra FC-15 interop；receive-pack report-status/CAS delete、filepath CASE batch、DB deadlock retry、RedLock 10ms、Snowflake env/Redis/hash（保留 6+8）。不覆盖压缩/range-read、Campsite identity、crates-sync、Media GC、monoui 客户端、共享 fixture crate（`DEFER-FC-01..06`）。 |
| [`plan-20260904.md`](plan-20260904.md) | N/A（storage-only trunk 产品 API 写文件；非 Mega PT） | **已完成**（AW-05 / v0.8.61） | AW-01..05：push_auth 门控 + MonoWriteQueue tip 落地；API 写后 git clone/pull 可见；compose 可继承黑盒脚本。不重开 CL/Cedar；不打开 SSH receive-pack。 |
...
| [`plan-20260912.md`](plan-20260912.md) | N/A（storage-only 提交后出站事件；非 Mega PT） | **收口中**（WH-01..WH-14 done/complete；WH-15 已实现于 v0.10.40 bump 提交，`remote-pending` 直至推送、tag 与 `docker` job 绿灯） | `[storage_events]` 默认关闭、storage-only token/none 可启用、restart-required。HTTPS HMAC 运输 + DNS 地址钉住 + 有界 emitter + 启动 secret 绑定 + CLI/service 清理尾段均已落地；六类来源 hook 全部挂上（`repo.push` / `oci.manifest.published` / `lfs.object.uploaded` / `lfs.media.finalized` / `agent_capture.events.committed` / `agent_capture.checkpoint.committed`）。2026-09-16 完成度复核追加的 WH-14（配置范围/规范名门）已于 2026-09-17 发布（v0.10.39）；WH-15（`installation_id` 记录字段与 drop 记账）已实现于 v0.10.40 bump 提交，待推送与 D 门。无 webhook CRUD/outbox/retry；presigned 直传与 media 静态 token 认证为 `DEFER-WH-01`/`DEFER-WH-02`。 |
